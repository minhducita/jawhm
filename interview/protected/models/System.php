...
class System extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('descriptions, keywords', 'required'),
			array('name, title', 'length', 'max'=>200),
			array('email_admin, timezone', 'length', 'max'=>100),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('name, title, email_admin, timezone, descriptions, keywords', 'safe', 'on'=>'search'),
		);
	}
