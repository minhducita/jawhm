	...
	if(!function_exists("getPlace")) {
		function getPlace() { 
			...
			foreach($listpara as $val) {
				...
				if(!empty($keyplace)) {
					return $places[$keyplace];
				}
			}
			for($i=1; $i<=6; $i++) {
				$keyplace = array_search($_SESSION["para".$i],$places);
				if(!empty($keyplace)) {
					return $places[$keyplace];
				}
			}
...
//retrieve data sent by the seminar menu
	$retrieved_data = array( 'last_msg_id' 		=> $_POST['last_msg_id'],
							 ...
							 'citybutton' 		=> $_POST['place'],//$_SESSION['para3'],//JN:change
							 'extra'			=> $_SESSION['para4']
							 );
	
    //JN:check
    //$placename = 'osaka';
    //$retrieved_data['citybutton'] = 'tokyo';
    //die(print_r($retrieved_data['citybutton']));
    //JN:end
...
//	$last_limit = $last_limit + 5;
	...
	try {
        ...
        $sql = 'SELECT count(*) as num FROM event_list WHERE k_use = 1 '.$free_query.' AND hiduke = DATE_SUB("' . $_POST['date'] . '", INTERVAL 0 DAY)' . $keyword . ' AND k_stat != 2 AND booking < pax ORDER BY hiduke, starttime, id ';
        $rs = $db->query($sql);
        ...
        $sql = 'SELECT id, hiduke, year(hiduke) as yy, month(hiduke) as mm, day(hiduke) as dd, date_format(starttime, \'%c月%e日 (%a) %k:%i\') as start, date_format(starttime, \'%k:%i\') as starttime, title, memo, place, k_use, k_title1, k_desc1, k_desc2, k_stat, free, pax, booking, group_color, indicated_option, broadcasting, country_code FROM event_list WHERE k_use = 1 '.$free_query.' AND hiduke = DATE_SUB("' . $_POST['date'] . '", INTERVAL 0 DAY)' . $keyword . $addfullsql . ' ORDER BY hiduke, starttime, id '.$SQL_limit;
		...
		while($row = $rs->fetch(PDO::FETCH_ASSOC))
		{
			...
			if ($c_img <> '')
				$c_img = '<p style="color:'.$p_color.'; margin-bottom:5px;">'.$c_img.'</p>';
			...
			if ($row['free'] && $_SESSION['mem_id'] == '') {
				$link = 'onclick="window.open(\'/seminar/ser/login/'.$row['id'].'\',\'_self\')"';
				...
				$link ='onclick="window.open(\'/seminar/ser/id/'.$row['id'].$add_area.'\',\'_self\')"';
			...
			$c_msg .= '<div class="detail"> '.'<br/><button '.$link.'class="btn btn-ser_m" value="セミナーに申し込む">　　セミナーに申し込む　　</button>';
            
			if(empty($placename)) {
				$placename = !empty($row['place'])?$row['place']:"tokyo";
			}
			if(file_exists($_SERVER['DOCUMENT_ROOT'].'/seminar/popup/'.$placename.'.html')) {
                ...
                //JN:change
                $c_msg .= '<div class="JN">';
                    include_once $_SERVER['DOCUMENT_ROOT'].'/seminar/popup/'.$placename.'.html';
                $c_msg .= '</div>';
                //JN:end
            }
            $c_msg .= '<br/>'.nl2br(str_replace('http', 'https', $c_desc)).'';

			$c_msg .= '<br/><br/><button '.$link.'class="btn btn-ser_m" value="セミナーに申し込む">　　セミナーに申し込む　</button>';
			$c_msg .= '</div>';
			$c_msg .= '</div></div>';
