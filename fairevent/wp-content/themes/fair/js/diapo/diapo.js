...
$('.pix_diapo').live('click',function(e){
	e.preventDefault();
			});
	...
	if(elem.length!=0){
		...
		if(navHover==true){	//if the option is true I show the next and prev button only on mouse over
			if(isMobile()){
				elem.bind('vmouseover',function(){
					$('#pix_prev, #pix_next').animate({opacity:1},200);
				});
				elem.bind('vmouseout',function(){
		...
		if(opts.pauseOnClick==true){	//if option is true I stop the slideshow if the user clicks on the slider
			selector.bind(clickEv,function(){
				autoAdv = false;
				elem.addClass('stopped');
				$('#pix_stop').fadeOut(100,function(){
					$('#pix_play').fadeIn(100);
			...
			$('.iframe_video').contents().find('body').bind('click',function(){
				autoAdv = false;
				elem.addClass('stopped');
				$('#pix_stop').fadeOut(100,function(){
					$('#pix_play').fadeIn(100);
		...
		if (($.browser.msie && $.browser.version < 9) || opts.loader == 'bar') {	//IE8- has some problems with canvas, I prefer to use a simple loading bar in CSS
			...
			if (canvas === null) {
				$('script[src="/jquery.hoverIntent.minified.js"]').remove();
				$('script[src="/diapo.js"]').remove();
				return;
			}
	...
	function nextSlide(nav){    //funzione per il fading delle immagini
		...
		switch(fx){
			...
			$.each(arr, function(index, value) {
				...
				if(navigation==true){
					$('#pix_prev').bind(clickEv,function(){
					...
					$('#pix_next').bind(clickEv,function(){
						if(!elem.hasClass('diaposliding')){
							var idNum = parseFloat($('div.diapocurrent',elem).index());
							clearInterval(u);
							imgFake();
				...
				if(isMobile()){
					elem.bind('swipeleft',function(event){
						if(!elem.hasClass('diaposliding')){
							var idNum = parseFloat($('div.diapocurrent',elem).index());
							clearInterval(u);
							imgFake();
					...
					elem.bind('swiperight',function(event){
						if(!elem.hasClass('diaposliding')){
							var idNum = parseFloat($('div.diapocurrent',elem).index());
							clearInterval(u);
							imgFake();
				...
				if(pagination==true){
					$('#pix_pag li').bind(clickEv,function(){
						if(!elem.hasClass('diaposliding')){
				...
				if(opts.thumbs==true){
					...
					$('#pix_pag li .pix_thumb').bind(clickEv,function(){
						if(!elem.hasClass('diaposliding')){
