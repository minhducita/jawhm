...
<?php
	...
	try {
		...
		$keyword .= ' and k_desc1 like \'%春のワーホリ・留学フェア%\'';
...
//			$youbi	= mb_ereg_replace('Sun', '日', $youbi);
			...
			if ($row['k_stat'] == 2)	{
				...
				if ($row['booking'] >= $row['pax'])	{
					$c_btn	= '<span class="button_yoyaku" onmouseover="this.style.backgroundColor=\'#0061D7\';" onmouseout="this.style.backgroundColor=\'navy\';" onclick="parent.fnc_yoyaku(this);" name="['.$place.'A]'.$title.'" uid="'.$row['id'].'">キャンセル待ち</span>';
				}else{
