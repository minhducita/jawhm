        ...
        {if $list|@count >= 1}
            {foreach item=item from=$list}
                {if $item.leave_country != '' && $item.leave_date !=''}
                    <li><a id="staffabroad_{$item.study_abroad_no}_staff/preparation/index">{$item.leave_date|date_format:"%G年%m月%d日"} のご渡航
                ...
                {else}
                    <li><a id="staffabroad_{$item.study_abroad_no}_staff/preparation/index"> 今回のご渡航</a></li>
                {/if}
            {/foreach}
