...
function previous_post_link_plus($args = '') {
	return adjacent_post_link_plus($args, '<div id="nav_arrow_left">&laquo;&nbsp;</div>%link', true);
}
function previous_post_link_plus_mobile($args = '') {
	return adjacent_post_link_plus($args, '<div class="nav-previous">%link</div>', true);
}
...
function next_post_link_plus($args = '') {
	return adjacent_post_link_plus($args, '<div id="nav_arrow_right">&nbsp;&raquo;</div>%link', false);
}
function next_post_link_plus_mobile($args = '') {
	return adjacent_post_link_plus($args, '<div class="nav-next">%link</div>', false);
}
...
//		Truncate the link title to nearest whole word under the length specified.
		...
		$anchor = '<a href="'.get_permalink($post).'" id="a_'.$rel.'" rel="'.$rel.'"'.$tooltip.'><div>';
		$link = str_replace('%title', $title, $r['link']);
		$link = str_replace('%date', $date, $link);
		$link = $anchor . $link . '</div></a>';
