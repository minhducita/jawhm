...
<div id="comments" class="comments-area"  >
	...
	<?php // You can start editing here -- including this comment! ?>
		...
		'author' =>
			...
			'" size="30"' . $aria_req . ' required title="この項目は必須項目です。"/></p>',

		  'email' =>
			'<p class="comment-form-email"><label for="email">' . __( 'メールアドレス', 'domainreference' ) . ' '.( $req ? '<span class="required">*</span>' : '' ) .'</label> 
			
			<input id="email"  name="email" type="text" value="' . esc_attr(  $commenter['comment_author_email'] ) .
			'" size="30"' . $aria_req . ' required title="入力したメールアドレスの形式が不正、または未入力です。"/></p>',

		  'url' =>'' ) ),

			'comment_field' => '<p>' .

						'<label for="comment">' . __( 'メッセージ' ) . ' ' .( $req ? '<span class="required">*</span>' : '' ) .'</label>' .

						'<textarea id="comment" name="comment" cols="45" rows="8" aria-required="true" required title="この項目は必須項目です。"></textarea>' .
