 ...
 */
	...
	if ($blog_id == 2) { // Australia Blog // オーストラリアブログの各学校の設定は以下から変更できます。
		...
		if ($p_id == 217) { // IH Brisbane
			$header_bg = '/wp-content/uploads/sites/2/2015/02/ihbri_header.gif';
			$header_name = '';
			$header_font_color = '';
			$footer_color = '';
		}
		if ($p_id == 15) { // 語学学校 インパクト School (Impact)
			$header_bg = '/wp-content/uploads/sites/2/2015/02/impact_header.gif';
			$header_name = '';
			$header_font_color = '';
			$footer_color = '';
		}
	...
	if ($blog_id == 3) { // Canada Blog // カナダブログの各学校の設定は以下から変更できます。
		...
		if ($p_id == 60) { // 語学学校 CPPC(Cornerstone Pan Pacific College) School
			$header_bg = '/wp-content/uploads/sites/3/2015/02/cppc_header.gif';
			$header_name = '';
			$header_font_color = '';
			$footer_color = '';
		}
