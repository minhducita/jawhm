 ...
 */
	...
	foreach ( $blogs as $b_id ) {
		...
		while($arrgs->have_posts()){
			...
			$content_post = mb_substr(get_the_excerpt(), 0,100);
		...
		foreach($jap_posts as $post_data){
			if ($x < 6) {
				echo '<li class="n_li"><a href="'. $post_data['get_permalink'] .'">';
					...
					//echo "<div class='imgContainer' style='background-image: url(" . $post_data['img_link'] . "); " . $post_data['thumb_css'] . "'>";
					...
					echo "<p class='n_content'>" . $post_data['content_post'] . "</p>";
