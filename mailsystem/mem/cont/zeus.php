...
*/
		...
		try {
			$stt = $db_mem->prepare('SELECT seq FROM kessai_seq');
			$stt->execute();
			$idx = 0;
			$cur_seq = '0';
			...
			$stt = $db_mem->prepare('UPDATE kessai_seq SET seq = '.$cur_num.' ');
			$stt->execute();
		...
		//先入力用
		$price = fnc_getpost('price');
		$price = is_null($price) ? "":$price;
		$name = fnc_getpost('name');
		$name = is_null($name) ? "":$name;
		$reason = fnc_getpost('reason');
		$reason = is_null($reason) ? "":$reason;
		$memo = fnc_getpost('memo');
		$memo = is_null($memo) ? "":$memo;
		$email = fnc_getpost('email');
		$email = is_null($email) ? "":$email;
		$year = fnc_getpost('year');
		$year = is_null($year) ? 2 : $year;
		$atena = fnc_getpost('atena');
		$atena = is_null($atena) ? 2 : $atena;
		...
		$body[] .= '
				...
				if (confirm(\'カード手動決済用のＵＲＬを発行します。よろしいですか？\'))	{
					...
					jQuery.ajax({
						...
						success: function(msg){
							...
							var url = jQuery("#res_net_card a").attr("href");
							jQuery("#edit_url").val(url);
							jQuery("#sendmail").show();
							jQuery("#btn_request").attr("disabled", "disabled");
							jQuery(".default-setting-btn").attr("disabled", "disabled");
						},
						error:function(){
							jQuery("#processing").hide();
							alert("通信エラーが発生しました。");
			...
			function fncsendmail()	{
				...
				if (confirm("支払方法設定ＵＲＬをメールでお送りします。よろしいですか？"))	{
					...
					jQuery.ajax({
						type: "POST",
						url: "'.$url_base.'data/zeus/card_payment_sendmail",
						data: "edit_email=" + jQuery("#edit_email").val() + "&edit_url=" + jQuery("#edit_url").val() + "&edit_name=" + jQuery("#namae").val() + "&edit_price=" + jQuery("#money").val() + "&edit_reason=" + jQuery("#youto").val() + "&edit_year=" + jQuery("#year").val() + "&edit_atena=" + jQuery("#atena").val(),
						success: function(msg){
							jQuery("#processing").hide();
							alert(msg);
							jQuery("#btn-sendmail").attr("disabled", "disabled");
						},
						error:function(){
							jQuery("#processing").hide();
							alert("通信エラーが発生しました。");
			...
			function fncsetfields(year){
				jQuery("#year").val(year);
				if(year == 2){
					jQuery("#money").val(2000);
					jQuery("#youto").val("【メンバー更新(2年)】");
				}else if(year == 3){
					jQuery("#money").val("5000");
					jQuery("#youto").val("【メンバー更新(3年)】");
				}
			}
			</script>
			...
			<div style="margin-bottom:5px;">
				<button id="year-btn-2" class="default-setting-btn" onclick="fncsetfields(2)" style="border-width:2px;border-style:outset;width:50px;margin-right:15px;">2年</button>
				<button id="year-btn-3" class="default-setting-btn" onclick="fncsetfields(3)" style="border-width:2px;border-style:outset;width:50px;margin-right:15px;">3年</button>
			</div>
			<form id="netcard_form" onsubmit="return false;">
				...
				<tr>
					<td>決済額</td><td><input type="text" name="money" id="money" value="'.$price.'" style="ime-mode:inactive;">(必須)　半角入力</td>
				</tr>
				<tr>
					<td>お名前</td><td><input type="text" name="namae" id="namae" value="'.$name.'" style="ime-mode:inactive;">(必須)　ローマ字入力（例：YAMADA TARO)</td>
				</tr>
				<tr>
					<td>適用</td><td><input type="text" name="youto" id="youto" value="'.$reason.'" style="ime-mode:active;">(必須)　（例：カナダビザ申請セット販売）</td>
				</tr>
				<tr>
					<td>メモ</td><td><input type="text" name="memo" id="memo" value="'.$memo.'" style="ime-mode:active;"></td>
				</tr>
				</table>
			...
			<div id="sendmail" style="display:none;">
				<table border="1">
				<tr><th>メール宛名</th><td><input type="text" size="30" value="'.$atena.'" id="atena">
				</td></tr>
				<tr><th>メール送信</th><td><input type="text" size="30" value="'.$email.'" id="edit_email">
				<input type="hidden" id="edit_url" value="">
				<input type="button" id="btn-sendmail" value="送信" onclick="fncsendmail();">
				</td></tr>
				</table>
			</div>
		';
	...
	case "payment":
		...
		try {
			$stt = $db_mem->prepare('SELECT id, email, namae, tel, state, orderid, orderdate, mailcheck FROM memlist WHERE id = "'.$id.'"');
			$stt->execute();
			$idx = 0;
			while($row = $stt->fetch(PDO::FETCH_ASSOC)){
				$idx++;
		...
		$url = 'https://member.jawhm.or.jp/mem2/payment.php?act=s3&userid='.$cur_id.'&email='.$cur_email.'&mailcheck='.$cur_mailcheck;
	...
	case "card_payment_sendmail":
		// 支払方法設定ＵＲＬメール送信
		$email = fnc_getpost('edit_email');
		$url = urldecode(fnc_getpost('edit_url'));
		$name = fnc_getpost('edit_name');
		$price = fnc_getpost('edit_price');
		$reason = fnc_getpost('edit_reason');
		$year = fnc_getpost('edit_year');
		$atena = fnc_getpost('edit_atena');

		// $type = fnc_getpost('edit_type');

		// 確認メールを送信
		$subject = "【".$atena." 様】日本ワーキング・ホリデー協会メンバー登録更新に関して";
		$mail  = '';
		$mail .= $atena .' 様';
		$mail .= chr(10);
		$mail .= chr(10);
		$mail .= 'こんにちは。';
		$mail .= chr(10);
		$mail .= '日本ワーキング・ホリデー協会でございます。';
		$mail .= chr(10);
		$mail .= 'この度はメンバー登録更新に関してお問い合わせいただき誠にありがとうございます。';
		$mail .= chr(10);
		$mail .= chr(10);
		if($year == 3){
			$mail .= '会員登録の更新は、お支払い確認日より'.$year.'年間延長となり更新料'.$price.'円がかかります。';
		}else{
			$mail .= '会員登録の更新は、ご登録日より'.$year.'年間延長となり更新料'.$price.'円がかかります。';
		}
		$mail .= chr(10);
		$mail .= '更新料をご入金いただき、確認ができ次第更新手続きを行い完了となります。';
		$mail .= chr(10);
		$mail .= chr(10);
		$mail .= 'つきましては、まず更新料'.$price.'円のご入金をお願い申し上げます。';
		$mail .= chr(10);
		$mail .= chr(10);
		$mail .= '【クレジットカードでのお支払い】';
		$mail .= chr(10);
		$mail .= '下記URLより手順に従い、更新料をお支払いいただきますようお願い致します。';
		$mail .= chr(10);
		$mail .= chr(10);
		$mail .= $reason;
		$mail .= chr(10);
		...
		$mail .= chr(10);
		$mail .= chr(10);
		$mail .= '上記URLはお客様のお名前で決済ができるように、手続きをさせていただいております。';
		$mail .= chr(10);
		$mail .= 'ご利用のクレジットカードの名義が異なる場合、上記URLでの決済ができかねますので、';
		$mail .= chr(10);
		$mail .= 'カード名義が異なる場合は、お手数ではございますが、';
		$mail .= chr(10);
		$mail .= '当協会へ再度、ご使用になるクレジットカードの名義をご連絡くださいませ。';
		$mail .= chr(10);
		$mail .= chr(10);
		$mail .= '確認のため、お振込が完了次第、当協会にメールにてご一報ください。';
		$mail .= chr(10);
		$mail .= chr(10);
		$mail .= chr(10);
		$mail .= 'ご不明な点などございましたらご連絡くださいませ。';
		$mail .= chr(10);
		$mail .= '何卒よろしくお願い致します。';
		$mail .= chr(10);
		$mail .= chr(10);
	...
	case "member":
		...
		try {
			$stt = $db_mem->prepare('SELECT id, email, namae, tel, state, orderid, orderdate FROM memlist WHERE id = "'.$id.'"');
			$stt->execute();
			$idx = 0;
			while($row = $stt->fetch(PDO::FETCH_ASSOC)){
				$idx++;
	...
	case "conv":
		...
		try {
			if ($new == 'new')	{
				...
				$stt = $db_mem->prepare('UPDATE memlist SET sid = "'.$dat_sid.'" where id = "'.$id.'" ');
				$stt->execute();
			}

			$stt = $db_mem->prepare('SELECT id, email, namae, tel, state, orderid, orderdate, sid, add1, add2, payment FROM memlist WHERE id = "'.$id.'"');
			$stt->execute();
			$idx = 0;
			while($row = $stt->fetch(PDO::FETCH_ASSOC)){
				$idx++;
...
//			'<form method="post" action="http://www.jawhm.or.jp/check.php" enctype="x-www.form-encoded" target="_blank" onsubmit="window.close();">'.
			'<form method="post" action="http://member.jawhm.or.jp/check.php" enctype="x-www.form-encoded" target="_blank" onsubmit="window.close();">'.
...
//		$stt2 = $db->prepare($sql);
		...
		$msg .= '
			<div style="padding:10px 10px 10px 10px; border: 1px dotted navy; margin: 10px 10px 10px 10px;">
				ネット決済用ＵＲＬは次の通りです。<br/>
				&nbsp;<br/>
				<a href="http://member.jawhm.or.jp/card/pay.php?auth='.$edit_authcd.'" target="_blank">http://member.jawhm.or.jp/card/pay/'.$edit_authcd.'</a><br/>
				&nbsp;<br/>
			</div>
		';
