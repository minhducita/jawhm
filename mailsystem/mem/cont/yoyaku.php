	...
	case "main":
		...
		$script .= '
			...
			function fnckuriage(id)	{
				if (confirm("このCXL待ち予約を繰上て予約確定状態に変更します。よろしいですか？"))	{
					jQuery("#processing").show();
					jQuery.ajax({
						type: "POST",
...
//		';
	...
	case "yoyaku_show":
		...
		if ($id == '')	{
			...
			try {
				$stt = $db->prepare('SELECT id, starttime, k_title1, pax, booking, waitting, place FROM event_list WHERE id = "'.$cur_seminarid.'"');
				$stt->execute();
				$idx = 0;
				while($row = $stt->fetch(PDO::FETCH_ASSOC)){
					$idx++;
					$cur_seminarname = "[".item_alias('area',$row['place'])."Z]".date("n月j日 (D) G:i～",strtotime($row['starttime']))." ".$row['k_title1'];
	...
	case "yoyaku_edit":
		...
		try {
			if ($edit_id == '')	{
				...
				$stt = $db->prepare('SELECT id, place, hiduke, year(hiduke) as y, month(hiduke) as m, day(hiduke) as d, date_format(hiduke,\'%w\') as yobi, date_format(starttime,\'%k:%i\') as sttime, k_title1, pax, booking, waitting, mailinfo, online_type, online_url FROM event_list WHERE id = "'.$edit_seminarid.'" ');
				$stt->execute();
				$idx = 0;
				$cur_id = '';
				while($row = $stt->fetch(PDO::FETCH_ASSOC)){
				...
				if ($edit_email <> '' && $edit_amail == 'on')	{
					...
					switch ($cur_place)	{
						case 'tokyo':
							$mail .='　東京会場';
							$mail .= chr(10);
							$mail .='　https://www.jawhm.or.jp/event/map?p=tokyo';
							$mail .= chr(10);
							break;
						case 'osaka':
							$mail .='　大阪会場';
							$mail .= chr(10);
							$mail .='　https://www.jawhm.or.jp/event/map?p=osaka';
							$mail .= chr(10);
							break;
						case 'nagoya':
							$mail .='　名古屋会場';
							$mail .= chr(10);
							$mail .='　https://www.jawhm.or.jp/event/map?p=nagoya';
							$mail .= chr(10);
							break;
						case 'fukuoka':
							$mail .='　福岡会場';
							$mail .= chr(10);
							$mail .='　https://www.jawhm.or.jp/event/map?p=fukuoka';
							$mail .= chr(10);
							break;
						case 'sendai':
							$mail .='　仙台会場';
						...
						case 'okinawa':
							$mail .='　沖縄会場';
							$mail .= chr(10);
							$mail .='　https://www.jawhm.or.jp/event/map?p=okinawa';
							$mail .= chr(10);
							break;
						case 'online':
							$mail .='　オンライン会場' . chr(10);
							if($cur_online_type == 1){
								$mail .='【YouTube】' . chr(10);
								$mail .= $cur_online_url . chr(10);
								$mail .= chr(10);
							}elseif ($cur_online_type == 2) {
								$mail .='【Zoom Cloud Meetings】' . chr(10);
								$mail .= $cur_online_url . chr(10);
								$mail .= chr(10);
								$mail .= '・パソコンから利用される場合は、アプリ等をダウンロードする必要なく下記リンクからそのまま利用できます。';
								$mail .= chr(10);
								$mail .= '・スマートフォン/タブレットからの方は、事前にZoomアプリをダウンロードして頂き上記リンクからご利用頂けます。';
								$mail .= chr(10);
								$mail .= chr(10);
								$mail .= chr(10);
								$mail .= "≪お願い≫";
								$mail .= chr(10);
								$mail .= "※開始3分前にはURLをクリックいただき、セミナーご参加の準備をお願いいたします。";
								$mail .= chr(10);
								$mail .= "※セミナーご参加時のお名前は必ず「フルネーム（ひらがな表記）」にしていただきますようお願い申し上げます。";
								$mail .= chr(10);
							}
							$mail .= chr(10);
						break;
					...
					$mail .= chr(10);
					$mail .= 'https://www.jawhm.or.jp/yoyaku/disp/'.$yoyakuno.'/'.md5($edit_email);
					$mail .= chr(10);
					$mail .= chr(10);
					$mail .= '◆このメールに覚えが無い場合◆';
					$mail .= chr(10);
	...
	case "yoyaku_csv":
		...
		try {
			...
			while($row = $stt->fetch(PDO::FETCH_ASSOC)){
				...
				$cur_weeks = "";
				$cur_school = "";
				$sst_mem = $db_mem->prepare("SELECT 1 as member FROM memlist where crmid = '".$cur_customid."' and crmid <> ''");
				$sst_mem->execute();
				while($row_mem = $sst_mem->fetch(PDO::FETCH_ASSOC)){
					$cur_mem = "◯";
				}
				$url = 'https://toratoracrm.com/crm/';
				$data = array(
				...
				$msg .= ',"'.$cur_status.'"';
				if($cur_contract <> ''){
					$msg .= ',"'.date("Y-m-d h:i",strtotime($cur_contract)).'"';
				}else{
					$msg .= ',"'.$cur_contract.'"';
				}
				$msg .= ',"'.$cur_weeks.'"';
	...
	case "yoyakulist_csv":
		...
		break;
	case "yoyakustatlist_csv":


		$seminarid = fnc_getpost('seminarid');

		$ini = parse_ini_file('../../../bin/pdo_mail_list.ini', FALSE);

		$cond  = ' event_list.id = "'.$seminarid.'"';
...
//		$cond .= ' or stat = "7" ';						// 不参加
		...
		try {
			$stt = $db->prepare("SELECT stat,namae,furigana,insdate,upddate,kyomi,jiki , event_list.k_title1 FROM entrylist inner join event_list on event_list.id = entrylist.seminarid WHERE seminarid = '". $seminarid . "' ORDER BY insdate");
			$stt->execute();
			$idx = 0;
			while($row = $stt->fetch(PDO::FETCH_ASSOC)){
				$idx++;
				...
				$msg .= ',"'.$cur_jiki.'"';
				$msg .= ',"'.$cur_insdate.'"';
				if ($cur_stat == 0 or $cur_stat == 1 or $cur_stat == 2) {
					$msg .= ',';
				} else {
					$msg .= ',"'.$cur_upddate.'"';
				}
	...
	case "yoyaku_sendmail":
		...
		try {
			$stt = $db->prepare('SELECT id, tel, email, kyomi, jiki, sonota, insdate, seminarid FROM entrylist WHERE id = '.$cur_id.' ORDER BY id');
			$stt->execute();
			$idx = 0;
			$cur_email = '';
			while($row = $stt->fetch(PDO::FETCH_ASSOC)){
			...
			$stt = $db->prepare('SELECT place, online_type, online_url FROM event_list WHERE id = '.$cur_seminarid.' AND place = \'online\' ORDER BY id');
			$stt->execute();
			$idy = 0;
			$cur_online_type = null;
			$cur_online_url = null;
			while($row = $stt->fetch(PDO::FETCH_ASSOC)){
				$idy++;
				$cur_online_type = $row['online_type'];
		...
		if ($cur_email <> '')	{
			...
			$mail .= '現在の予約状態を確認する場合は、以下のURLを表示してください。';
			$mail .= chr(10);
			$mail .= 'https://www.jawhm.or.jp/yoyaku/disp/'.$cur_id.'/'.md5($cur_email);
			$mail .= chr(10);
			$mail .= chr(10);
			$mail .= '◆このメールに覚えが無い場合◆';
			$mail .= chr(10);
	...
	case "yoyaku_kuriage":
		...
		try {
			...
			// メール送信
			$subject = 'イベント（セミナー）ご予約確定のお知らせ';
			$body1  = '';
			$body1 .= 'ご予約頂きました以下のイベント（セミナー）にご参加頂けるようになりました。';
			$body1 .= chr(10);
			$body1 .= chr(10);
			$body1 .= 'なお、このご連絡をもちまして、お客様のご予約は「確定」されておりますのでご安心ください。';
			$body1 .= chr(10);
			$body1 .= '（通常必要なご予約の確定をする必要はありません。）';
			$body1 .= chr(10);
			$body1 .= chr(10);
			$body1 .= '今後、このご予約をキャンセルする場合は、以下のURLからお願い致します。';
			$body1 .= chr(10);
			$body1 .= chr(10);
			$body1 .= 'では、当日会場でお待ちしております。お気を付けてお越しください。';
			$body2  = '';
...
function sendmail($subject, $body1, $body2, $cur_namae, $cur_place, $cur_y, $cur_m, $cur_d, $cur_yobi, $cur_sttime, $cur_title1, $cur_yoyakuid, $cur_email)	{
	...
	$body .= chr(10);
	$body .= 'https://www.jawhm.or.jp/yoyaku/disp/'.$cur_yoyakuid.'/'.md5($cur_email);
	$body .= chr(10);
	$body .= '';
	$from = mb_encode_mimeheader(mb_convert_encoding("日本ワーキングホリデー協会","JIS"))."<info@jawhm.or.jp>";
