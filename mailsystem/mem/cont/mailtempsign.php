...
switch ($data_param) {
    ...
    case "show":
        ...
        if (is_string($cur_keycd) && $cur_keycd == 'create') {
            ...
            try {
                ...
                while ($row = $stt->fetch(PDO::FETCH_ASSOC)) {
                    $idx++;
                    $cur_text1 = stripslashes($row['text1']);     // Fromメアド
            ...
            <div id="div_mailtemp_edit" style="display:none; margin:10px 20px 10px 20px; font-size:10pt; cursor:default;">
                ...
                <form id="form_mailtemp_edit">
                    ...
                    <table style="font-size:10pt;">
                        ...
                        <tr>
                            <td class="label" width="120">Fromメアド</td>
    ...
    case "showtestmail":
        ...
        try {
            ...
            while ($row = $stt->fetch(PDO::FETCH_ASSOC)) {
                $id = $row['id'];
                $mail_sign[$id] = $row['text1'];     // Fromメアド
            }
        } catch (PDOException $e) {
            die($e->getMessage());
        }
    ...
    case "sendmail":
        ...
        try {
            ...
            $stt->execute();
            while ($row = $stt->fetch(PDO::FETCH_ASSOC)) {
                $mail_sign = stripslashes($row['text3']);     // Fromメアド
            }
        } catch (PDOException $e) {
            die($e->getMessage());
        }
