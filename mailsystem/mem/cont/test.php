	...
	case "list":
		...
		$script .= '
			...
			function fncSereventList()	{
				...
				jQuery.ajax({
					type: "POST",
					url: "'.$url_base.'data/test/event_ser",
			...
			function fnceventShow(id)	{
				...
				jQuery.ajax({
					type: "POST",
					url: "'.$url_base.'data/test/event_show",
					data: "id=" + id,
					success: function(msg){
						jQuery("#processing").hide();
			...
			function fnceventPost(formname)	{
				...
				jQuery.ajax({
					type: "POST",
					url: "'.$url_base.'data/test/event_edit",
					data: jQuery("#" + formname).serialize(),
					success: function(msg){
						jQuery("#processing").hide();
						res = eval(msg);
			...
			function fncdel(formname)	{
				...
				jQuery.ajax({
					type: "POST",
					url: "'.$url_base.'data/test/event_del",
					data: jQuery("#" + formname).serialize(),
					success: function(msg){
						jQuery("#processing").hide();
						res = eval(msg);
			...
			function fncgo()	{
				...
				if (chk == \'\')	{
					...
					jQuery.ajax({
						type: "POST",
						url: "'.$url_base.'data/test/event_go",
						data: "id=" + chk,
						success: function(msg){
							jQuery("#processing").hide();
							jQuery("#res_event_edit").html(msg);
			...
			if (goforuse == 1)	{
					jQuery.ajax({
						type: "POST",
						url: "'.$url_base.'data/test/event_goforuse",
						success: function(msg){
							res = eval(msg);
							if (res[0].result == \'END\')	{
			...
			if (goforuse == 1)	{
					jQuery.ajax({
						type: "POST",
						url: "'.$url_base.'data/test/event_goforschool",
						success: function(msg){
							res = eval(msg);
							if (res[0].result == \'END\')	{
								jQuery("#lbl_goforuse").html(res[0].cnt);
			...
			function fncSereventCsv()	{
				...
				jQuery.ajax({
					type: "POST",
					url: "'.$url_base.'csv/test/event_csv",
					data: jQuery("#form_event_list").serialize(),
					success: function(msg){
						jQuery("#processing").hide();
						res = eval(msg);
						if (res[0].result == \'OK\')	{
							if (confirm(res[0].msg + "出力しますか？"))	{
								window.open(\''.$url_base.'csv/test/event_csv/?type=out&\' + jQuery("#form_event_list").serialize(), \'_blank\', \'width=200,height=200\');
								return false;
							}
						}else{
							alert(res[0].msg);
			...
			function fncSerKariCsv()	{
				...
				jQuery.ajax({
					type: "POST",
					url: "'.$url_base.'csv/test/kari_csv",
					data: jQuery("#form_event_list").serialize(),
					success: function(msg){
						jQuery("#processing").hide();
						console.log(msg);
						...
						if (res[0].result == \'OK\')	{
							if (confirm(res[0].msg + "出力しますか？"))	{
								window.open(\''.$url_base.'csv/test/kari_csv/?type=out&\' + jQuery("#form_event_list").serialize(), \'_blank\', \'width=200,height=200\');
								return false;
							}
						}else{
							alert(res[0].msg);
			...
			function fncshowdesc(id, div)	{
				...
				jQuery.ajax({
					type: "POST",
					url: "'.$url_base.'data/test/event_showdesc",
					data: "id=" + id,
					success: function(msg){
						jQuery("#processing").hide();
						jQuery("#" + div).html(msg);
	...
	case "event_edit":
		...
		try {
			if ($edit_id == '')	{
				...
				$sql .= ' hiduke, starttime, endtime, place, preuse, k_title1, k_title2, k_desc1, k_desc2, k_stat, free, type, pax, new, seminar_name, short_description, country_code, indicated_option, broadcasting, group_color, ongoogle, mailinfo, beginer, tantousha_name, tantousha_mail ';
				$sql .= ') VALUES (';
				$sql .= ' :hiduke, :sttime, :edtime, :place, :preuse, :k_title1, :k_title2, :k_desc1, :k_desc2, :k_stat, :free, :type, :pax, :new, :seminar_name, :short_description, :country_code, :indicated_option, :broadcasting, :group_color, :ongoogle, :mailinfo, :beginer, :tantousha_name, :tantousha_mail ';
				$sql .= ')';
				$stt2 = $db->prepare($sql);
