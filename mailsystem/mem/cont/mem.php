...
switch ($data_param) {
    case "main":
        ...
        $script .= '
					...
					success: function(msg){
						...
						if (res[0].result == \'OK\')	{
							if (confirm(res[0].msg + "出力しますか？"))	{
								window.open(\'' . $url_base . 'csv/mem/mem_csv/?type=out&\' + jQuery("#form_mem_list").serialize(), \'_blank\', \'resizable=yes, scrollbars=yes, titlebar=yes, width=200, height=200, top=10, left=10\');
								return false;
							}
						}else{
							alert(res[0].msg);
			...
			// 銀行入金（登録日変更）処理
			function fncmemBank(formname)	{
				if (!confirm(\'支払日（入金日）を設定し、登録日を変更します。よろしいですか？\'))	{
					return false;
				}
				var objform = document.getElementById(formname);
				var obj = objform.getElementsByTagName(\'input\');
				...
				jQuery.ajax({
					type: "POST",
					url: "' . $url_base . 'data/mem/mem_bank",
					data: jQuery("#" + formname).serialize(),
					success: function(msg){
						jQuery("#processing").hide();
						res = eval(msg);
        ...
        if ($id == '') {
            ...
            try {
                ...
                while ($row = $stt->fetch(PDO::FETCH_ASSOC)) {
                    ...
                    $cur_dupchk = $row['dupchk'];
					$cur_validuntil = $row['validuntil'];
					$cur_romaji = TextUtil::convertKKtoR($row['furigana']);
                }
...
-->				<input type=button value="仮カード" onclick="fncsetprecard(\'' . $cur_id . '\');jQuery.unblockUI();return false;">
				<input type=button value="ＰＷＤ" onclick="if(confirm(\'パスワードがクリアされます。よろしいですか？\')){fncsetrepwd(\'' . $cur_id . '\');jQuery.unblockUI();}return false;">
				<script>
					// infomail対応
					function fncsetprecard(id)	{
						var radio = document.getElementsByName(\'kyotenmail\');
						var text;
						var kyoten;
						var x ;
						for( x=0; x< radio.length ; x++){
							text = radio[x].outerHTML;
							if (text.indexOf(\'checked\') >= 0){
								kyoten = radio[x].value;
							}
						}
						window.open(\'' . $url_base . 'data/mem/precard/\' + id + \'/\'  + kyoten, \'_blank\', \'width=990,height=800\');
					}
					function fncsetrepwd(id)	{
						var radio = document.getElementsByName(\'kyotenmail\');
						var text;
						var kyoten;
						var x ;
						...
						window.open(\'' . $url_base . 'data/mem/repwd/\' + id + \'/\'  + kyoten, \'_blank\', \'width=990,height=800\');
					}
					function fncgotocardpayment(){
						url = \''.$url_base.'main/zeus\';
						url = url + \'?name='.$cur_romaji.'\';
						url = url + \'&price=2000\';
						url = url + \'&reason='.urlencode("【メンバー更新(2年)】").'\';
						url = url + \'&email='.urlencode($cur_email).'\';
						url = url + \'&year=2\';
						url = url + \'&atena='.$cur_namae.'\';
						window.open(url,\'card_payment\');
						return false;
					}

				</script>

				<div id="kyotenmail" hidden>
				<input type=radio name="kyotenmail" value="KT" '.($cur_kyoten == "KT" ?"checked":"").'> 東京
				<input type=radio name="kyotenmail" value="KO" '.($cur_kyoten == "KO" ?"checked":"").'> 大阪
				<input type=radio name="kyotenmail" value="KN" '.($cur_kyoten == "KN" ?"checked":"").'> 名古屋
				<input type=radio name="kyotenmail" value="KF" '.($cur_kyoten == "KF" ?"checked":"").'> 福岡
				<input type=radio name="kyotenmail" value="KK" '.($cur_kyoten == "KK" ?"checked":"").'> 沖縄
				</div>
			';
            $quest = '
				...
				<table>
					...
					<tr>
						...
						<td class="infield" colspan="3"><span id="div_state">' . fnc_state($cur_state) . '</span>
					        ...
					        if ($cur_orderid == '' && $cur_state == '1') {
					            $msg .= '
									<input type="button" value="銀行入金（取扱注意）" onclick="fncmemBank(\'form_mem_edit\');">
								';
					        }
				$msg .= '
							&nbsp;&nbsp;&nbsp;有効期限：' . field_text('edit_validuntil', 12, $cur_validuntil) . '
						</td>
					</tr>
					<tr>
					...
					<tr>
						...
						<td class="infield" colspan="3">
							...
							<input type="button" onclick="fncgotocardpayment()" value="メンバー更新" style="margin-left:15px;height:22px;';
							if($cur_state <> '9'){
								$msg .= 'display:none;';
							}
							$msg .= '">
        ...
        $msg .= '	<tr>
						<td class="label" nowrap>
							重要メモ<br/>
							&nbsp;<input type="checkbox" name="edit_attchk" ' . $attchk . '>注意情報<br/>
							&nbsp;<input type="checkbox" name="edit_dupchk" ' . $dupchk . '>重複確認
						</td>
						<td class="infield" colspan="3">
    ...
    case "mem_edit":
        ...
        $edit_dupchk = fnc_getpost('edit_dupchk');
        if ($edit_dupchk == 'on') {
            $edit_dupchk = '1';
        } else {
            $edit_dupchk = '0';
        }
        $edit_validuntil = fnc_getpost('edit_validuntil');

        try {
            if ($edit_id == '') {
                // 会員番号採番
                $stt = $db_mem->prepare('SELECT max(id) as maxid FROM memlist');
                $stt->execute();
                $idx = 0;
                $cur_id = 'JW000000';
                while ($row = $stt->fetch(PDO::FETCH_ASSOC)) {
                ...
                $sql .= ' id, email, namae, furigana, gender, birth, pcode, add1, add2, add3, tel, state, mailsend, password, indate, insdate, upddate, memo, memcard, orderdate, attchk , dupchk , kyoten, validuntil ';
                $sql .= ') VALUES (';
                $sql .= ' :id, :email, :namae, :furigana, :gender, :birth, :pcode, :add1, :add2, :add3, :tel, :state, :mailsend, :password, :indate, :insdate, :upddate, :memo, :memcard, :orderdate, :attchk , :dupchk, :kyoten, :validuntil ';
                $sql .= ')';
                $stt2 = $db_mem->prepare($sql);
                ...
                $stt2->bindValue(':dupchk', $edit_dupchk);
                $stt2->bindValue(':kyoten', $edit_kyoten);
                $stt2->bindValue(':validuntil', $edit_validuntil);
                $stt2->execute();
                $arr = array(
                    array(
                        "result" => "OK",
                ...
                $sql .= ', dupchk = ' . $edit_dupchk;
                if ($edit_validuntil == '') {
                    $sql .= ', validuntil = NULL';
                } else {
                    $sql .= ', validuntil = "' . $edit_validuntil . '"';
                }
    ...
    case "mem_ser":
        ...
        try {
            $stt = $db_mem->prepare('SELECT id, namae, furigana, state, crmid, mailcheck, memcard, kyoten, attchk, dupchk FROM memlist WHERE ' . $cond . ' ORDER BY id');
            $stt->execute();
            $idx = 0;
            while ($row = $stt->fetch(PDO::FETCH_ASSOC)) {
                $idx++;
    ...
    case "mem_csv":
        ...
        try {
            $stt = $db_mem->prepare('SELECT id ,email ,namae ,furigana ,gender ,date_format(birth,\'%Y/%m/%d\') as dob ,pcode ,add1 ,add2 ,add3 ,add4 ,job ,tel ,country ,gogaku ,purpose ,know ,state ,date_format(indate,\'%Y/%m/%d\') as nyukaidate ,mailcheck ,orderid ,date_format(orderdate,\'%Y/%m/%d\') as carddate ,crmid ,memo ,memcard, kyoten, payment,date_format(validuntil,\'%Y/%m/%d\') as untildate FROM memlist WHERE ' . $cond . ' ORDER BY id');
            $stt->execute();
            $idx = 0;
            while ($row = $stt->fetch(PDO::FETCH_ASSOC)) {
                $idx++;
    ...
    case "mem_yamato":
        ...
        try {
            $stt = $db_mem->prepare('SELECT id ,email ,namae ,furigana ,gender ,date_format(birth,\'%Y/%m/%d\') as dob ,pcode ,add1 ,add2 ,add3 ,add4 ,job ,tel ,country ,gogaku ,purpose ,know ,state ,date_format(indate,\'%Y/%m/%d\') as nyukaidate ,mailcheck ,orderid ,date_format(orderdate,\'%Y/%m/%d\') as carddate ,crmid ,memo ,memcard, kyoten, payment FROM memlist WHERE ' . $cond . ' ORDER BY id');
            $stt->execute();
            $idx = 0;
            while ($row = $stt->fetch(PDO::FETCH_ASSOC)) {
                $idx++;
    ...
    case "card":
        ...
        $id = $param[3];
        try {
            $stt = $db_mem->prepare('SELECT id, email, namae, tel, state, indate FROM memlist WHERE id = "' . $id . '"');
            $stt->execute();
            $idx = 0;
            while ($row = $stt->fetch(PDO::FETCH_ASSOC)) {
                $idx++;
    ...
    case "card3":
        ...
        $id = $param[3];
        try {
            $stt = $db_mem->prepare('SELECT id, email, namae, tel, state, indate, password, mailcheck FROM memlist WHERE id = "' . $id . '"');
            $stt->execute();
            $idx = 0;
            while ($row = $stt->fetch(PDO::FETCH_ASSOC)) {
                $idx++;
...
//		$pdf->Image('http://www.jawhm.or.jp/images/memcard01.png', 0, 0, 90.0, 56.0);
        ...
        $pdf->Text(57, 45, mb_convert_encoding($cur_namae, 'SJIS-win'));
        $pdf->SetFont('SJIS', '', 7);
...
//        $pdf->SetTextColor(255, 255, 255);
        ...
        $pdf->Text(57, 44, mb_convert_encoding($cur_namae, 'SJIS-win'));
        $pdf->SetFont('SJIS', '', 5);
    ...
    case "precard":
        ...
        $kyoten = $param[4];
        try {
            $stt = $db_mem->prepare('SELECT id, email, namae, tel, state, orderid, orderdate, indate, payment FROM memlist WHERE id = "' . $id . '"');
            $stt->execute();
            $idx = 0;
            while ($row = $stt->fetch(PDO::FETCH_ASSOC)) {
                $idx++;
		...
		switch($kyoten){
			case 'KT':
				$kyotenmail = "sodan@jawhm.or.jp";
				$x = 4;
				break;

			case 'KO':
				$kyotenmail = "sodan-osaka@jawhm.or.jp";
				$x = 16;
				break;

			case 'KN':
				$kyotenmail = "sodan-nagoya@jawhm.or.jp";
				$x = 19;
				break;

			case 'KF':
				$kyotenmail = "sodan-fukuoka@jawhm.or.jp";
				$x = 21;
				break;

			case 'KK':
				$kyotenmail = "sodan-okinawa@jawhm.or.jp";
				$x = 21;
				break;
			default:
				$kyotenmail = "info@jawhm.or.jp";
				$x = 0;
				break;
		}
        $size = array(210, 297);
        ...
        $pdf->Text(30, 85, mb_convert_encoding('お名前　　　　　：　' . $cur_namae, 'SJIS-win'));
...
//        $pdf->Text(110, 185, mb_convert_encoding('TEL : 03-6304-5858  E-Mail: info@jawhm.or.jp', 'SJIS'));
        $pdf->Text(110 - $x, 185, mb_convert_encoding('TEL : 03-6304-5858  E-Mail: '.$kyotenmail, 'SJIS'));
        ...
        $pdf->Text(120, 252, mb_convert_encoding($cur_namae, 'SJIS-win'));
        $pdf->SetFont('SJIS', '', 10);
	...
	$kyoten = $param[4];
        try {
            $stt = $db_mem->prepare('UPDATE memlist SET password = :password WHERE id = :id ');
            $stt->bindValue(':id', $id);
            $stt->bindValue(':password', md5($newpwd));
            $stt->execute();
            $stt = $db_mem->prepare('SELECT id, email, namae, tel, state, orderid, orderdate, indate FROM memlist WHERE id = "' . $id . '"');
            $stt->execute();
            $idx = 0;
            while ($row = $stt->fetch(PDO::FETCH_ASSOC)) {
                $idx++;
        ...
        $pdf->SetTextColor(0, 0, 0);
        $pdf->SetFont('SJIS', '', 14);
        $pdf->Text(15, 36, mb_convert_encoding($cur_namae . ' 様', 'SJIS-win'));
        ...
        $pdf->Text(30, 120, mb_convert_encoding('お名前　　　　　：　' . $cur_namae, 'SJIS-win'));
...
//        $pdf->Text(110, 264, mb_convert_encoding('TEL : 03-6304-5858  E-Mail: info@jawhm.or.jp', 'SJIS'));
        $pdf->Text(110 - $x, 264, mb_convert_encoding('TEL : 03-6304-5858  E-Mail: '.$kyotenmail, 'SJIS'));
    ...
    case "show_quest":
        ...
        try {
            $stt = $db_mem->prepare('SELECT id, country, gogaku, purpose, know FROM memlist WHERE id = "' . $edit_id . '"');
            $stt->execute();
            $idx = 0;
            while ($row = $stt->fetch(PDO::FETCH_ASSOC)) {
                $idx++;
    ...
    case "mem_cardsend":
        ...
        try {
            ...
            while ($idx < count($arr_id)) {
                $stt = $db_mem->prepare('SELECT id, namae, crmid, memcard, state FROM memlist WHERE id = "' . $arr_id[$idx] . '"');
                $stt->execute();
                while ($row = $stt->fetch(PDO::FETCH_ASSOC)) {
                    $cur_id = $row['id'];
                    $cur_namae = $row['namae'];
                ...
                if ($dataerr == 0) {
                    ...
                    $tbl_msg .= '<td>';
                    $tbl_msg .= '<input type="text" size="30" name="send_' . $no . '" id="send_' . $no . '" onfocus="fncgetfocus(this);" onblur="fnclostfocus(this);" onkeydown="if(event.keyCode == 13){jQuery(\'#send_' . ($no + 1) . '\').focus();}" style="ime-mode:inactive;">';
    ...
    case "send_crm":
        ...
        try {
            $stt = $db_mem->prepare('SELECT id, email, namae, furigana, gender, birth, year(birth) as yy, month(birth) as mm, day(birth) as dd, pcode, add1, add2, add3, tel, state, indate, mailsend, orderid, orderdate, crmid, crmdate, job, country, gogaku, purpose, know FROM memlist WHERE id = "' . $edit_id . '"');
            $stt->execute();
            $idx = 0;
            while ($row = $stt->fetch(PDO::FETCH_ASSOC)) {
                $idx++;
