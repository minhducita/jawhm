...
switch ($data_param)	{
	...
	case "list":
		...
		$script .= '
			...
			function fncExportOrderMemberCsv(id_selected = "")	{
				if(confirm("当件数は 1件 です。出力しますか？")){
					jQuery("#processing").show();
					window.open(\''.$url_base.'csv/yoyakusha/export_csv?type=out&ids_selected=\' + id_selected, \'_blank\', \'width=200,height=200\');
					jQuery("#processing").hide();
				}
			}

			// セミナーの予約状態出力
			function fncYoyakuCsv(id)	{
				jQuery("#processing").show();
				window.open(\''.$url_base.'csv/yoyaku/yoyakustatlist_csv/?seminarid=\' + id  , \'width=200,height=200\');
				jQuery("#processing").hide();
			}
		...
		if ($data_param == 'list')	{
			...
			$body_title[] .= 'イベント検索';
			$ser_cond = '';
			$ser_cond2 = '<label style="float: right; padding-right: 20px; width: 254px; display: inline-block;">キーワード無<input type=checkbox name="ser_keynone" value="none"></label>';
		}
		$body[] .= '
			<form id="form_event_list">
				<table class="w100">
					...
					<tr>
						<td style="text-align:right; width: 100px;">セミナー区分</td>
						<td>
							<label for="free_p1"><input type="checkbox"  id="free_p1" name="free_p1" value="0">無料</label>　
							<label for="free_p2"><input type="checkbox"  id="free_p2" name="free_p2" value="1">有料</label>　
							'.$ser_cond.'
							'.$ser_cond2.'
						</td>
					</tr>
					<tr>
						...
						<td>
							...
							<label for="ser_p9"><input type="checkbox" id="ser_p9" name="ser_p9" value="kumamoto">熊本</label>　
							<br>
					...
					<tr>
						<td style="text-align:right;">検索対象</td>
						<td>
							<select name="ser_schoolap">
								<option value="all" selected>全て</option>
								<option value="none">通常のみ</option>
								<option value="school">学校のみ</option>
							</select>　
							<label>除外キーワード ' . field_text('ser_notkeyword', 20, '') . '</label>
						</td>
					</tr>
	...
	case "event_show":
		...
		$cur_id = $id;
                ...
                try {
                    $sql = "SELECT * FROM mailtext WHERE keycd LIKE 'mail_remind' ORDER BY id";
                    $stt = $db->prepare($sql);
                    $stt->execute();
                    while ($row = $stt->fetch(PDO::FETCH_ASSOC)) {
                        $id_mail_temp = $row['id'];
                        $list_mail_remind_template[$id_mail_temp] = stripslashes($row['text1']);
                    }
                } catch (PDOException $e) {
                    die($e->getMessage());
                }
		if ($id == '')	{
			...
			try {
				...
				while($row = $stt->fetch(PDO::FETCH_ASSOC)){
					...
					$cur_schoolap = $row['schoolap'];
					//TA show name_charge_of
					$cur_tantousha_name = $row['tantousha_name'];

					$cur_tantousha_mail = $row['tantousha_mail'];
					//TA end show name_charge_of
					$cur_remind = $row['remind'];
		...
		if (isset($cur_schoolap) && $cur_schoolap == '1')	{
			$cur_schoolap = ' checked ';
		}else{
			$cur_schoolap = '';
		}
		if ($cur_new == '1')	{
		...
		$msg = '
			<div id="div_event_edit" style="display:none; margin:10px 20px 10px 20px; font-size:10pt; cursor:default; overflow-y:auto; height: 580px;">
				...
				<table>
					<tr>
						...
						<td class="infield" width="360" colspan="3">
							<label><input type="checkbox" id="edit_use" name="edit_use" '.$cur_use.'>本掲載</label>
							<label><input type="checkbox" id="edit_schoolap" name="edit_schoolap" '.$cur_schoolap.'>学校</label>
							<label><input type="checkbox" id="edit_new" name="edit_new" '.$cur_new.'>フォローメール不要</label>
							<label><input type="hidden" name="edit_id" value="'.$cur_id.'"></label>
						</td>
					</tr>
					<tr>
		...
		$msg .= '			</select>
					...
					<tr>
						...
						<td class="infield" colspan="3">
							...
							<label><input type="radio" name="edit_place" value="kumamoto"'.$place['kumamoto'].'>熊本</label>
							<br>
					...
					<tr>
						...
						<td class="infield" colspan="3">
							<label><input type=radio name="edit_stat" value="0"'.$stat[0].'>空き</label>　
							<label><input type=radio name="edit_stat" value="1"'.$stat[1].'>混雑</label>　
							<label><input type=radio name="edit_stat" value="2"'.$stat[2].'>満席</label>　
						</td>
					</tr>
					<tr>
						<td class="label" nowrap>remindメール</td>
						<td class="infield" colspan="3">
						 	'.dropdown_list('edit_remind', $cur_remind, $list_mail_remind_template, array('empty' => 'メールを送信しない')) . '
						</td>
					</tr>
					<tr>
						<td class="label" nowrap>種別</td>
						<td class="infield" colspan="3">
							<label><input type=radio name="edit_free" value="0"'.$free[0].'>無料</label>　　
							<label><input type=radio name="edit_free" value="1"'.$free[1].'>有料</label>　
					...
					<tr>
						...
						<td class="infield">
							<label><input type=radio name="edit_broadcasting" value="0"'.$broadcasting[0].'>無し</label>　
							<label><input type=radio name="edit_broadcasting" value="1"'.$broadcasting[1].'>中継</label>
						</td>
					</tr>
					...
					<tr>
						...
						<td class="infield" colspan="3">
							<textarea name="edit_mailinfo" cols="50" rows="3" style="font-size:9pt; height: 35px;">'.$cur_mailinfo.'</textarea>
						</td>
					</tr>
					</table>
			</td><td>
				<table>
					...
					<tr>
						...
						<td class="infield">
							<textarea name="edit_desc1" cols="80" rows="17" style="font-size:9pt; height: 232px;">'.$cur_desc1.'</textarea>
						</td>
					</tr>
					<tr>
	...
	case "event_edit":
		...
		$edit_schoolap = fnc_getpost('edit_schoolap');
		...
		$edit_remind = fnc_getpost('edit_remind');
		...
		try {
			if ($edit_id == '')	{
				...
				$sql .= ' hiduke, starttime, endtime, place, k_use, k_title1, k_title2, k_desc1, k_desc2, k_stat, free, type, pax, new, seminar_name, short_description, country_code, indicated_option, broadcasting, group_color, ongoogle, mailinfo, beginer, schoolap, tantousha_name, tantousha_mail,remind, online_type, online_url ';
				$sql .= ') VALUES (';
				$sql .= ' :hiduke, :sttime, :edtime, :place, :k_use, :k_title1, :k_title2, :k_desc1, :k_desc2, :k_stat, :free, :type, :pax, :new, :seminar_name, :short_description, :country_code, :indicated_option, :broadcasting, :group_color, :ongoogle, :mailinfo, :beginer, :schoolap, :tantousha_name, :tantousha_mail,:remind, :online_type, :online_url ';
				$sql .= ')';
				$stt2 = $db->prepare($sql);
				...
				$stt2->bindValue(':schoolap'	, $edit_schoolap);
				...
				$stt2->bindValue(':remind', $edit_remind);
				...
				$sql .= ', beginer = :beginer';
				$sql .= ', schoolap = "'.$edit_schoolap.'"';

				//TA save person charge of
				$sql .= ', tantousha_name = "' . $edit_tantousha_name . '"';
				$sql .= ', tantousha_mail = "' . $edit_tantousha_mail . '"';
				//TA save person charge of
				$sql .= ', remind = "'.$edit_remind.'"';
                $sql .= ', online_type = "' . $edit_online_type . '"';
                $sql .= ', online_url = "' . $edit_online_url . '"';
	...
	case "event_ser":
		...
		$ser_schoolap = fnc_getpost('ser_schoolap');
		$ser_notkeyword = fnc_getpost('ser_notkeyword');
		...
		if ($ser_p1 <> '' || $ser_p2 <> '' || $ser_p3 <> '' || $ser_p4 <> '' || $ser_p5 <> '' || $ser_p6 <> '' || $ser_p7 <> '' || $ser_p8 <> '' || $ser_p9 <> '' || $ser_p10 <> '')	{
			$cond .= ' and ( 1=0 ';
			...
			if ($ser_p9 <> '')	{	$cond .= ' or place ="'.$ser_p9.'" ';	}
		...
		if ($ser_notkeyword <> '')	{
			$cond .= ' and ( ';
			$cond .= ' k_title1 not like "%'.$ser_notkeyword.'%" and ';
			$cond .= ' k_title2 not like "%'.$ser_notkeyword.'%" and ';
			$cond .= ' k_desc1 not like "%'.$ser_notkeyword.'%" and ';
			$cond .= ' k_desc2 not like "%'.$ser_notkeyword.'%" ) ';
		}
		if ($ser_keynone <> '')	{
			$cond .= ' and k_desc2 = "" ';
		}
		if ($ser_schoolap == 'none')	{
			$cond .= ' and schoolap = 0 ';
		}
		if ($ser_schoolap == 'school')	{
			$cond .= ' and schoolap = 1 ';
		}
		if ($ser_evtno <> '')	{
		...
		if ($ser_param == 'list')	{
			require_once '_includes/functions.php';
			// 予約状況一覧
			$cond .= ' and ( k_use = 1 or schoolap = 1 ) ';
			...
			try {
				$stt = $db->prepare('SELECT id, hiduke, place, k_use, k_title1, k_title2, k_desc1, k_desc2, k_stat, pax, booking, waitting, schoolap FROM event_list WHERE '.$cond.' and ongoogle = 1 ORDER BY hiduke, starttime, place');
				$stt->execute();
				$idx = 0;
				while($row = $stt->fetch(PDO::FETCH_ASSOC)){
					$idx++;
			...
			//		$msg .= '<input type=button class="button_rev" value="予約" onclick="fncyoyakuShow(\''.$cur_id.'\');">';
					...
					if ($cur_schoolap == '1')	{
						$msg .= '<span style="font-weight:bold; background-color:yellow; font-size:96%;">学</span>';
					}
					$msg .= '</td>';
			...
			$msg .= '<tr>';
			$msg .= '<th>編集</th>';
			$msg .= '<th width="105">日付<br/>開始</th>';
			$msg .= '<th width="75">開催地<br/>状態</th>';
			$msg .= '<th width="">タイトル</th>';
//CSV--------------------------------------------------
			$msg .= '<th width="20">予約<br/>状態</th>';
//CSV--------------------------------------------------
			$msg .= '</tr>';

			try {
				$stt = $db->prepare('SELECT id, hiduke, date_format(starttime,\'%H:%i\') as sttime, date_format(endtime,\'%H:%i\') as edtime, place, k_use, k_title1, k_title2, k_desc1, k_desc2, k_stat, schoolap FROM event_list WHERE '.$cond.' and ongoogle = 1 ORDER BY hiduke, starttime, place');
				$stt->execute();
				$idx = 0;
				while($row = $stt->fetch(PDO::FETCH_ASSOC)){
					...
					// if(!in_array($row["id"],array())){
						...
						if ($cur_schoolap == '1')	{
							$msg .= '<br/><span style="font-weight:bold; background-color:yellow; font-size:96%;">学校</span>';
						}
						$msg .= '</td>';
						$msg .= '<td><a id="list'.$cur_id.'" href="#'.$cur_id.'" onclick="fncshowdesc(\''.$cur_id.'\',\'div_'.$cur_id.'\');">'.$cur_title.'</a><div id="div_'.$cur_id.'"></div></td>';
	//CSV--------------------------------------------------
						$msg .= '<td><input type=button class="button_csv" value=" 出力" onclick="fncYoyakuCsv(\''.$cur_id.'\');"></td>';
	//CSV--------------------------------------------------
	...
	case "event_showdesc":
		...
		try {
			...
			while($row = $stt->fetch(PDO::FETCH_ASSOC)){
				...
				$msg .= '　　<input type="button" class="button_list" value="　一覧" onclick="window.location.href=\''.$url_base.'main/yoyaku?seminarid='.$cur_id.'\'">';
                $msg .= '    <input type="button" class="button_save" value="CSV" onclick="fncExportOrderMemberCsv(' . $cur_id . ');">';
				$msg .= '</td>';
				$msg .= '</tr>';
				$msg .= '<tr><td colspan="3">&nbsp;時刻 ： '.$cur_sttime.'　～　'.$cur_edtime.'</td></tr>';
				$msg .= '<tr><td colspan="2">単独表示</td><td colspan="1"><label style="padding-right:10px;">ＰＣ</label><input type="text" size="50" value="https://www.jawhm.or.jp/s/go/'.$cur_id.'"><br><label style="padding-right:10px;">携帯</label><input type="text" size="50" value="https://www.jawhm.or.jp/seminar/ser/id/'.$cur_id.'"></td></tr>';
				$msg .= '<tr><td colspan="2">同時開催</td><td colspan="1"><input type="text" size="60" value="https://www.jawhm.or.jp/s/go2/'.$cur_id.'"></td></tr>';
				$msg .= '</table>';
				$msg .= '';
			}
		} catch (PDOException $e) {
	...
	case "event_csv":
		...
		if ($ser_p1 <> '' || $ser_p2 <> '' || $ser_p3 <> '' || $ser_p4 <> '' || $ser_p5 <> '' || $ser_p6 <> '' || $ser_p7 <> '' || $ser_p8 <> '' || $ser_p9 <> '' || $ser_p10 <> '')	{
			...
			if ($ser_p8 <> '')	{	$cond .= ' or place ="'.$ser_p8.'" ';	}
			if ($ser_p9 <> '')	{	$cond .= ' or place ="'.$ser_p9.'" ';	}
			if ($ser_p10 <> '')	{	$cond .= ' or online_type > 0 ';	}
			$cond .= ' ) ';
		}
		if ($ser_keyword1 <> '')	{
			$cond .= ' and ( ';
