	...
	<div data-role="content">
		...
		<nav>
			<ul data-role="listview" data-inset="true" data-theme="a" data-dividertheme="a">
				<li data-role="list-divider">オンラインセミナー状況</li>
...
<?
	...
	try {
		...
		while($row = $rs->fetch(PDO::FETCH_ASSOC)){
			...
			if ($row['free'] == 1)	{
				if ($mem_id == '')	{
					...
					if ($row['k_stat'] == 2)	{
						...
						if ($row['booking'] >= $row['pax'])	{
							$c_btn	= '<input class="button_yoyaku" type="button" value="キャンセル待ち" onclick="fnc_yoyaku(this);" title="[WEB]'.$title.'" uid="'.$row['id'].'">';
						}else{
							$c_btn	= '<input class="button_yoyaku" type="button" value="メンバー専用予約" onclick="fnc_yoyaku(this);" title="[WEB]'.$title.'" uid="'.$row['id'].'">';
						}
					}
				}
			}else{
				if ($row['k_stat'] == 2)	{
					...
					if ($row['booking'] >= $row['pax'])	{
						...
						$c_btn	= '<input class="button_yoyaku" type="button" value="予約" onclick="fnc_yoyaku(this);" title="[WEB]'.$title.'" uid="'.$row['id'].'">';
					}
				}
			}
			...
			$c_msg .= $c_img.'';
			$c_msg .= '<h3>'.$month.'月'.$day.'日　'.$row['starttime'].'～　オンライン会場　　'.'予約状況：'.$row['booking'].' ( '.$row['waitting'].' ) '.$row['pax'].'</h3>';
