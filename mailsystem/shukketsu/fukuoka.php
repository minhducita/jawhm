	...
	<div data-role="content">
		...
		<nav>
			<ul data-role="listview" data-inset="true" data-theme="a" data-dividertheme="a">
				<li data-role="list-divider">福岡セミナー状況</li>
...
<?
	...
	try {
		...
		// パラメータ確認
		$keyword  = ' and ( place = \'fukuoka\' ';
		$keyword .= ' or k_title1 like \'%福岡%\' ';
		$keyword .= ' or k_title2 like \'%福岡%\' ';
		$keyword .= ' or k_desc1 like \'%福岡%\' ';
		$keyword .= ' or k_desc2 like \'%福岡%\' ';
		$keyword .= ' ) ';
		...
		while($row = $rs->fetch(PDO::FETCH_ASSOC)){
			...
			$c_msg .= $c_img.'';
			$c_msg .= '<h3>'.$month.'月'.$day.'日　'.$row['starttime'].'～　福岡会場　　'.'予約状況：'.$row['booking'].' ( '.$row['waitting'].' ) '.$row['pax'].'</h3>';
