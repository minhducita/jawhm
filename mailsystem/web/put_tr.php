...
<h1 style="color:white;background-color:orange;font-size:22px;">トラトラ（東京）の一括送信</h1>
<?php
...
if($_POST['submit'] != NULL){
	...
	try {
		...
		$rs = $db->query('SELECT vmail as email, vname1 as namae , vcheck FROM maillist WHERE vsend = "1" AND vtype = "tr"');
		$cnt = 0;
		while($row = $rs->fetch(PDO::FETCH_ASSOC)){
			...
			$headers = array(
				'From' => mb_encode_mimeheader('トラトラ東京','ISO-2022-JP').'<tokyo@tora-tora.net>',
			...
			$q->put('tokyo@tora-tora.net', $row['email'], $headers, $body);
		}
...
?>
	<form method="POST" action="put_tr.php" onsubmit="return(confirm('本当に送信しますか？'));">
		件名：<br />
...
---------
株式会社TRAVEL&TRAVEL【トラトラ東京】
 Mail：tokyo@tora-tora.net
