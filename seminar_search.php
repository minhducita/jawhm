...
function calender_show($year, $month)	{
	...
	print '<table style=" border:none; border-collapse:collapse; background-color:#cccccc; font-size: 12pt; color: #666666;" id="'.$year.$month.'" class="'.$year.$month.'">';
	print '<tr>';
	print '<td colspan="7" style="height:18px;text-align:center;background-color:#EEEEEE;color: #666666;">'.$year.'年'.$month.'月</td></tr>';
	print '<tr>';
	print '<td style="height:18px;width:40px;text-align:center;background-color:#FF3300;color: #FFFFFF;">日</td>';
	print '<td style="width:40px;text-align:center;background-color:#C7D8ED;color: #666666;">月</td>';
	print '<td style="width:40px;text-align:center;background-color:#C7D8ED;color: #666666;">火</td>';
	print '<td style="width:40px;text-align:center;background-color:#C7D8ED;color: #666666;">水</td>';
	print '<td style="width:40px;text-align:center;background-color:#C7D8ED;color: #666666;">木</td>';
	print '<td style="width:40px;text-align:center;background-color:#C7D8ED;color: #666666;">金</td>';
	print '<td style="width:40px;text-align:center;background-color:#A6C0E1;color: #666666;">土</td>';
	print '</tr>';
...
//		$link = '<a href="#'.$year.substr('00'.$month,-2).substr('00'.$i,-2).'">'.@$cal[$year.$month.$i].$i.'</a><br/>';
	...
	if ($change == "on")	{
		$change = '<td style="text-align:center; height:18px; white-space:nowrap; background-color:#ffcc99; color: #666666;">'.$link.'</td>';
	}
	...
	if ($change == "")	{
		...
		elseif($w==6){
			// 土曜日
			$change = '<td bgcolor="#FFFFFF" style="text-align:center; height:18px; white-space:nowrap; background-color:#ffffff; color: #666666;">'.$link.'</td>';
		}
		else{
			$change = '<td bgcolor="#FFFFFF" style="text-align:center; height:18px; white-space:nowrap; background-color:#ffffff; color: #666666;">'.$link.'</td>';
		}
...
//		$rs = $db->query('SELECT id, year(hiduke) as yy, month(hiduke) as mm, day(hiduke) as dd, title, memo, place, k_use, k_title1, k_desc1, k_stat FROM event_list WHERE k_use = 1 AND hiduke >= "'.date("Y/m/d",strtotime("-1 week")).'"  ORDER BY hiduke, id');
		...
		while($row = $rs->fetch(PDO::FETCH_ASSOC)){
			...
			if ($row['place'] == 'tokyo')	{
				...
				$c_msg  = '<tr><td style="white-space:nowrap; vertical-align:top;">'.$row['starttime'].'～ <img src="images/sa05.jpg" alt="" /><a target="_blank" href="/event/map/?p=tokyo">東京会場</a></td><td nowrap style="vertical-align:top;">'.$c_btn.'</td>';
			...
			if ($row['place'] == 'osaka')	{
				...
				$cal[$year.$month.$day] .= '<img src="images/sa01.jpg" alt="" />';
				$c_msg  = '<tr><td  style="white-space:nowrap;vertical-align:top;">'.$row['starttime'].'～ <img src="images/sa01.jpg" alt="" /><a target="_blank" href="/event/map/?p=osaka">大阪会場</a></td><td nowrap style="vertical-align:top;">'.$c_btn.'</td>';
				$c_msg .= '<td>'.$row['k_title1'].'</td></tr><tr><td colspan="4">';
				$c_msg .= ''.$c_img.'<div class="open" style="margin-left:150px;">セミナー詳細はココをClick!!</div>';
				$c_msg .= '<div class="det" style="margin:5px 0 10px 50px; padding: 5px 0 13px 12px; display:none; border-left:1px dotted navy; border-bottom: 1px dotted navy;">'.$c_title.nl2br($c_desc).'</div></td></tr>';
				$cal_msg[$year.$month.$day] .= $c_msg;
			...
			if ($row['place'] == 'fukuoka')	{
				...
				$cal[$year.$month.$day] .= '<img src="images/sa03.jpg" alt="" />';
				$cal[$year.$month.$day] .= '<img src="images/sa05.jpg" alt="" />';
				$c_msg  = '<tr><td  style="white-space:nowrap;vertical-align:top;">'.$row['starttime'].'～ <img src="images/sa03.jpg" alt="" /><a target="_blank" href="/event/map/?p=fukuoka">福岡会場</a></td><td nowrap style="vertical-align:top;">'.$c_btn.'</td>';
				$c_msg .= '<td>'.$row['k_title1'].'</td></tr><tr><td colspan="4">';
				$c_msg .= ''.$c_img.'<div class="open" style="margin-left:150px;">セミナー詳細はココをClick!!</div>';
				$c_msg .= '<div class="det" style="margin:5px 0 10px 50px; padding: 5px 0 13px 12px; display:none; border-left:1px dotted navy; border-bottom: 1px dotted navy;">'.$c_title.nl2br($c_desc).'</div></td></tr>';
				$cal_msg[$year.$month.$day] .= $c_msg;
			...
			if ($row['place'] == 'sendai')	{
				...
				$cal[$year.$month.$day] .= '<img src="images/sa02.jpg" alt="" />';
				$c_msg  = '<tr><td nowrap style="vertical-align:top;">'.$row['starttime'].'～ <img src="images/sa02.jpg" alt="" />仙台会場</td><td nowrap style="vertical-align:top;">'.$c_btn.'</td>';
				$c_msg .= '<td>'.$row['k_title1'].'</td></tr><tr><td colspan="4">';
				$c_msg .= ''.$c_img.'<div class="open" style="margin-left:150px;">セミナー詳細はココをClick!!</div>';
				$c_msg .= '<div class="det" style="margin:5px 0 10px 50px; padding: 5px 0 13px 12px; display:none; border-left:1px dotted navy; border-bottom: 1px dotted navy;">'.$c_title.nl2br($c_desc).'</div></td></tr>';
				$cal_msg[$year.$month.$day] .= $c_msg;
			...
			if ($row['place'] == 'toyama')	{
				...
				$cal[$year.$month.$day] .= '<img src="images/sa02.jpg" alt="" />';
				$c_msg  = '<tr><td style="white-space:nowrap;vertical-align:top;">'.$row['starttime'].'～ <img src="images/sa02.jpg" alt="" />富山会場</td><td nowrap style="vertical-align:top;">'.$c_btn.'</td>';
				$c_msg .= '<td>'.$row['k_title1'].'</td></tr><tr><td colspan="4">';
				$c_msg .= ''.$c_img.'<div class="open" style="margin-left:150px;">セミナー詳細はココをClick!!</div>';
				$c_msg .= '<div class="det" style="margin:5px 0 10px 50px; padding: 5px 0 13px 12px; display:none; border-left:1px dotted navy; border-bottom: 1px dotted navy;">'.$c_title.nl2br($c_desc).'</div></td></tr>';
				$cal_msg[$year.$month.$day] .= $c_msg;
			...
			if ($row['place'] == 'okinawa')	{
				...
				$cal[$year.$month.$day] .= '<img src="images/sa02.jpg" alt="" />';
				$c_msg  = '<tr><td nowrap style="vertical-align:top;">'.$row['starttime'].'～ <img src="images/sa02.jpg" alt="" /><a target="_blank" href="/event/map/?p=okinawa">沖縄会場</a></td><td nowrap style="vertical-align:top;">'.$c_btn.'</td>';
				$c_msg .= '<td>'.$row['k_title1'].'</td></tr><tr><td colspan="4">';
				$c_msg .= ''.$c_img.'<div class="open" style="margin-left:150px;">セミナー詳細はココをClick!!</div>';
				$c_msg .= '<div class="det" style="margin:5px 0 10px 50px; padding: 5px 0 13px 12px; display:none; border-left:1px dotted navy; border-bottom: 1px dotted navy;">'.$c_title.nl2br($c_desc).'</div></td></tr>';
				$cal_msg[$year.$month.$day] .= $c_msg;
...
<span style="font-size:11pt;text-align:center;display:block;">
