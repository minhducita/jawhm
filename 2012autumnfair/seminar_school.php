...
<?php
	...
	try {
		...
		$keyword .= ' and k_desc1 like \'%秋の留学・ワーキングホリデーフェア2012%\'';
		...
		while($row = $rs->fetch(PDO::FETCH_ASSOC)){
			...
			$place  = $row['place'];
			if ($place == 'tokyo')	$place = '東京';
			if ($place == 'osaka')	$place = '大阪';
			if ($place == 'fukuoka')	$place = '福岡';
			if ($place == 'okinawa')	$place = '沖縄';
			if ($place == 'event')		$place = 'イベント';
