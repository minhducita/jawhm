...
<?php
	...
	try {
		...
		$keyword .= ' and k_desc1 like \'%☆Working Holiday Very Merry Christmas!! 2013☆%\'';
		$keyword .= ' and hiduke >= DATE_SUB(CURDATE(),INTERVAL 0 DAY) ';
		$rs = $db->query('SELECT id, year(hiduke) as yy, month(hiduke) as mm, day(hiduke) as dd, date_format(starttime, \'<b>%c月%e日 (%a) </b> %k:%i\') as start, date_format(starttime, \'%a\') as youbi, date_format(starttime, \'%k:%i\') as starttime, title, memo, place, k_use, k_title1, k_desc1, k_stat, free, pax, booking, type FROM event_list WHERE k_use = 1 '.$keyword.' ORDER BY hiduke, starttime, place, id');
		$cnt = 0;
...
//			$youbi	= mb_ereg_replace('Sun', '日', $youbi);
			...
			if ($row['k_stat'] == 2)	{
				...
				if ($row['booking'] >= $row['pax'])	{
					$c_btn	= '<span class="button_yoyaku" onmouseover="this.style.backgroundColor=\'#0061D7\';" onmouseout="this.style.backgroundColor=\'navy\';" onclick="parent.fnc_yoyaku(this);" name="['.$place.'A]'.$title.'" uid="'.$row['id'].'">仮予約</span>';
				}else{
					$c_btn	= '<span class="button_yoyaku" onmouseover="this.style.backgroundColor=\'#0061D7\';" onmouseout="this.style.backgroundColor=\'navy\';" onclick="parent.fnc_yoyaku(this);" name="['.$place.'A]'.$title.'" uid="'.$row['id'].'">予約</span>';
				}
			}
			...
			if ($c_img <> '')	{
				$c_img = '<div style="color:red; font-size:11pt; font-weight:bold;">'.$c_img.'</div>';
			}

			$c_msg  = '<tr><td nowrap style="vertical-align:top;">'.$c_img.''.$c_btn.'&nbsp;&nbsp;'.$start.'&nbsp;'.$place.'&nbsp;</td>';
			$c_msg .= '';
			...
			if ($cnt == 1)	{
				echo '<table style="font-size:10pt; margin: -5px 0px 10px 10px;">';
			}
