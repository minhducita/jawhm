...
function calender_output($cal, $i,$w,$year,$month,$day,$place_name,$keyword,$selected_day=0,$date_limit_beginning=1,$half_month=false,$all_white=false)
{
	...
	*/
	...
	while($row = $rs->fetch(PDO::FETCH_ASSOC))
	{
		...
		$listing_data .= "</span><div class=\"description_seminar ".$fullybooked."\" ".$style_description.">".$decription."</div>"
			. "<div onclick='getListEvent(".$year.",".$month.",".$i.",this)' id='".$row['id']."'  class=\"name_seminar ".$fullybooked."\" ".$style_name." onmouseover=\"Tip('".$text_tooltip."', BGCOLOR, 'WHITE', TITLE, '".$seminar_booked.$seminar_name."', TITLEBGCOLOR, '".$title_bg_color."', TITLEFONTCOLOR, '".$title_color."', BORDERCOLOR, '".$color."', BORDERSTYLE, '".$border_style."', PADDING, 9, WIDTH, -300)\" onmouseout=\"UnTip()\">".$seminar_name."</div>";
