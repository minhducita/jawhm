...
$direct_root = $_SERVER['DOCUMENT_ROOT'];
require_once $direct_root."/seminar_module/seminar_db.php";
require_once $direct_root."/seminar_module/seminar_assets.php";
require_once $direct_root."/seminar_module/seminar_assets_list.php";
require_once $direct_root."/seminar_module/seminar_where.php";
require_once $direct_root."/seminar_module/seminar_event_list.php";
require_once $direct_root."/seminar_module/seminar_api/functions_api.php";
require_once $direct_root."/seminar_module/jp-holiday.php";
require_once $direct_root."/seminar_module/seminar_db_stub.php";
...
class SeminarModule
{
	...
	*/
	...
	function __construct($config = array())
	{
		...
		$tmp_config['calendar']['use_area'] = 'on';
	...
	private function _init()
	{
		...
		if ($this->_config['view_mode'] == 'calendar') {
			...
			if(empty($this->_config["active_event_list_init"])) {
				// パラメータに応じたセミナーリストを取得
	...
	private function _get_seminar_list()
	{
		...
		*/
		...
		foreach ($list as $row) {
			...
			if ($row['free'] == 1)	{
				if (@$_SESSION['mem_id'] == '')	{
					$c_title = '<div style="margin: 10px 0 10px 0; padding:5px 20px 5px 20px; border: 1px solid navy;">【こちらはメンバー様限定セミナーです】<br/>メンバー登録を行って頂くとご予約が可能となります。<a href="./mem2/register.php">登録はこちらからどうぞ</a><br/>メンバーの方は、ログインするとご予約が可能となります。</div>';
				}
			}
			$c_desc = $row['k_desc1'];
			$c_img = '';
			...
			if (!$this->pc_view && $this->_header_obj->computer_use() === false) {
				// spの時は、強制的にser以下に飛ばす
				$click_js = "window.location='/seminar/ser/id/" . $row['id'] . "'";
				if ($row['free'] == 1 && @$_SESSION['mem_id'] == '') {
					$click_js = "window.location='/seminar/ser/login/" . $row['id'] . "'";
				} else {
					if (empty($this->_config[$this->_config['view_mode']]['use_area']) || $this->_config[$this->_config['view_mode']]['use_area'] == 'off' || $this->_config[$this->_config['view_mode']]['use_area'] == 'no' || $this->_config[$this->_config['view_mode']]['use_area'] == 'false' || $this->_config[$this->_config['view_mode']]['use_area'] == '0') {
					} else {
						if ($row['place'] == 'event' && strpos($this->_place_name, $row['k_desc2']) === false) {
							$click_js = "window.location='/seminar/ser/id/" . $row['id'] . "/area'";
						} elseif ($row['place'] !== 'event' && $row['place'] !== $this->_place_name) {
							$click_js = "window.location='/seminar/ser/id/" . $row['id'] . "/area'";
						}
					}
...
<div class="det" style="border-color: rgb(255, 99, 71); display: block;width:460px;margin-left:0px;">' . nl2br($this->_douji_info[0]['k_desc1']) . '</div>
		...
		foreach ($place_list as $k => $p) {
			...
			for ($i = 0; $i < $diff; $i++) $p .= '　';
			$output .= '<input type="button" style="margin-bottom:10px;" class="button_submit" id="" value=" ' . $p . '" onClick="window.location=\'/seminar/seminar?num=' . $k . '&view=1#calendar_start\'"><br>';
		}
		$output .= '</div>
...
';
	...
	/* 
		Author: Ly Phuoc Nam.
		GET $param: day , month , year , placename.
		Description: show detail event list.
		update: 16/02/2016.
	*/
	...
	public function get_event_list_show(){
		$date = date("Y-m-d",strtotime($_GET['year']."-".$_GET['month']."-".$_GET["day"]));
		$day = $_GET['day'];
		$month = $_GET['month'];
		$where_day = "and hiduke = '".$date."'";
		$where_eid = "and id = ".$_GET['est'];
		
		$keyword = $this->access_keyword_event_list().$where_day.$where_eid;
		$keyword = str_replace("or k_desc1 like '%初心者%'","",$keyword);
	
		$list = $this->_db->get_event_list($keyword);
		$event_list = "";
		$cnt = 0;
		$html_event  = "";
		foreach ($list as $row) {
			...
			if ($row['free'] == 1)	{
				if (@$_SESSION['mem_id'] == '')	{
					$c_title = '<div style="margin: 10px 0 10px 0; padding:5px 20px 5px 20px; border: 1px solid navy;">【こちらはメンバー様限定セミナーです】<br/>メンバー登録を行って頂くとご予約が可能となります。<a href="http://'.$_SERVER['SERVER_NAME'].'/mem2/register.php">登録はこちらからどうぞ</a><br/>メンバーの方は、ログインするとご予約が可能となります。</div>';
				}
			}
			$c_desc = $row['k_desc1'];
			$c_img = '';
			...
			$calendar_day = date('n月j日', $print_today);
			$season 	  = date($yobi[date('w', $print_today)], $print_today);
			...
			$flag = '';
			if (!empty($row['country_code'])) {
				$flag = '<img src = "'.$urlimg."flag/".strtolower($row['country_code']).'.png" >';
			}
			$css = '<link rel="stylesheet" href="'.$domain.'/api/seminar/css/new-popup.css" type="text/css" media="all">';
			$html_event = $css."<div class='newpopup'><div class='nheader flex'>";
				$html_event .= "<div class='lefpopup'>";
				$html_event .= '<div class="title"><div class="buttonleft"></div><p>'.$season.'</p><div class="buttonright"></div></div>';
				$html_event .= '<p class="content">'.$calendar_day.' </p>';
				$html_event .= "</div>";
				$html_event .= "<div class='rightcontent'>";
				$html_event .= "<div class='righttitle'><p> ".$flag." <span class='fa fa-clock-o'></span>".$row['starttime']."～ ".$row['k_title1'].$option_information."</p>".$c_img."</div>";
				$html_event .= "</div>";
			$html_event .= "</div>";
			/*html content*/
			$html_event .= "<div class='clear'></div> <div class='maincontent'>
				<h3>".$row['k_title1']."</h3>
				<div class='flex'><img src='".$urlimg."icon1.png'><p>内容</p></div>
				<div class='clear'></div><div class='textcontent'>".$c_title.nl2br($c_desc)."</div>
				<div class='buttonright'> ".$c_btn." </div><Div class='clear'></div>
			</div></div>";
			$js = '<script> 
				function seminarInfo(){
					if((jQuery(window).width()-800)/2 >10){
						jQuery(".newpopup .nheader").removeClass("nheader-m");
						jQuery(".newpopup .nheader").addClass("flex");
					}else{
						jQuery(".newpopup .nheader").addClass("nheader-m");
						jQuery(".newpopup .nheader").removeClass("flex");
					}
				}
				seminarInfo();
