...
!empty($_POST['acao'])?header('Access-Control-Allow-Origin:http://'.$_POST['acao']):"";
	...
	*/
	...
	private $_config = array(
		...
		'calendar' => array(
			...
			'use_area' => '', // 地方セミナー対応
			'place_active' => 'active',
			'place_list' => array('online', 'tokyo', 'osaka', 'nagoya', 'fukuoka', 'okinawa', 'kyoto', 'omiya', 'hiroshima', 'kumamoto', 'toyama'),
			'place_default' => '',
	...
	private $pc_view = true;
        ...
        //JN:add property
        public $jn_day_selected = 0;
        public $jn_day_limit = 14;
        //JN:end
		public $show_limit = 0;
	/**
	 * コンストラクタ
	...
	function __construct($config = array())
	{
		...
		$tmp_config = array();
		if (!empty($_SESSION['seminar_config2'])) {
	...
	private function _init()
	{
		...
		if((!empty($_GET['pc_view']) && $_GET['pc_view'] == true) || !empty($_POST['pc_view'])){
			$this->pc_view = true;
		}else{
			$this->pc_view = false;
		}
			...
			if (isset($_GET['navigation'])) {
				...
				if (!empty($this->_config['start_date'])) {
					...
					if (count($date_list) == 3) {
                        ...
                        //JN:start:add code
                        if($this->jn_day_selected <= 0)
                        if(@isset($_POST['jn_day_selected']))
                        {
                            $this->jn_day_selected = $_POST['jn_day_selected']; //: date('j');
                            $this->jn_day_limit = @isset($_GET['day_limit']) ? $_GET['day_limit'] : 14; //number day to load seminar
                        }
                        else if(@isset($_GET['day_selected']))
                        {
                            $this->jn_day_selected = $_GET['day_selected']; //: date('j');
                            $this->jn_day_limit = @isset($_GET['day_limit']) ? $_GET['day_limit'] : 14; //number day to load seminar
                        }
                        else if(@isset($_POST['selected_day']))
                        {
                            $this->jn_day_selected = $_POST['selected_day'];
                            $this->jn_day_limit = @isset($_GET['day_limit']) ? $_GET['day_limit'] : 14; //number day to load seminar
                        }
                        else if(@isset($_GET['day']))
                        {
                            $this->jn_day_selected = $_GET['day'];
                            $this->jn_day_limit = @isset($_GET['day_limit']) ? $_GET['day_limit'] : 14; //number day to load seminar
                        }
                        else if($this->_month == date('n'))
                        {
                            $this->jn_day_selected = date('j');
                            $this->jn_day_limit = @isset($_GET['day_limit']) ? $_GET['day_limit'] : 14; //number day to load seminar
                        }
                        else
                        {
                             $num_day_month = date("t", mktime(0,0,0,$this->_month,1,$this->_year));
                             $day_week_first = date('w', mktime(0,0,0,$this->_month, 1, $this->_year));
                             $day_week_last = date('w', mktime(0,0,0,$this->_month, $num_day_month, $this->_year));
                             $this->jn_day_selected = 1;
                             $this->jn_day_limit = @isset($_GET['day_limit']) ? $_GET['day_limit'] : ($num_day_month + (6-$day_week_last) + $day_week_first); //number day to load seminar
                        }
                        if(isset($_POST['month_date']))
                        {
                            $this->_month = $_POST['month_date'];
                        }
                        if(isset($_POST['year_date']))    
                        {
                            $this->_year = $_POST['year_date'];
                        }
                        //$this->_selected_day = $this->jn_day_selected;
                        
                        //die($this->jn_day_selected);
                        //JN:end
		...
		*/
		...
		foreach ($list as $row) {
			...
			if ($row['indicated_option'] == 1) {
				$indication = '<span class="icon_info_osusume"></span>';
			} elseif ($row['indicated_option'] == 2) {
				$indication = '<span class="icon_info_shinchyaku"></span>';
			} elseif ($row['indicated_option'] == 3) {
				$indication = '<span class="icon_info_chumoku">注目</span>';
			}
			...
			$option_information = '<span class="icon_info'.$icon_live.'"></span>'.$indication.'';
			...
			if ($row["id"] <> 35423){
				$c_desc = str_replace('http','https',$row['k_desc1']);
			}
			$c_img = '';
			...
			if ($row['free'] == 1 && @$_SESSION['mem_id'] == '') {
				...
				if (empty($this->_config[$this->_config['view_mode']]['use_area']) || $this->_config[$this->_config['view_mode']]['use_area'] == 'off' || $this->_config[$this->_config['view_mode']]['use_area'] == 'no' || $this->_config[$this->_config['view_mode']]['use_area'] == 'false' || $this->_config[$this->_config['view_mode']]['use_area'] == '0') {
					...
					if ($row['place'] == 'event' && !empty($this->_place_name) && !empty($row['k_desc2']) && strpos($this->_place_name, $row['k_desc2']) === false) {
                        ...
                        //SPの時の予約ボタンの記述
			if (!$this->pc_view && $this->_header_obj->computer_use() === false) {
				// spの時は、強制的にser以下に飛ばす
			...
			if (!$this->pc_view && $this->_header_obj->computer_use() === false && $this->_config['use_mode'] == 'widget') {
				// spの時は、強制的にser以下に飛ばす
			...
			$this->_calendar[$year.'-'.$month.'-'.$day] .= '<img src="https://'.$_SERVER['SERVER_NAME'].'/images/sa05.jpg">';
			...
			if($row['place'] == 'event') {
				...
				$c_msg  .= '<a target="_blank" href="https://'.$_SERVER['SERVER_NAME'].'/event/map/?p='.$row['place'].'">'.$this->_translate_city($row['place']).'会場</a>';
...
//		}
		...
		//print_r($this->_config['list']['keyword']);
		if (!empty($this->_config['list']['keyword'])) {
			//echo "sdfkjgkgk";
			if (is_array($this->_config['list']['keyword']))	{
				
				//Minhquyen -- 19/12/2016
				if(strpos($_SERVER['REQUEST_URI'], 'fairevent') !== false)
				{
					$this->_keyword .= ' and ( ';
					for($i=0; $i<count($this->_config['list']['keyword']); $i++){
						
						if ($this->_config['list']['keyword'][$i] <> ''){
							if($i < count($this->_config['list']['keyword']) - 1){
								$this->_keyword .= ' (k_desc2 like \'%' . $this->_config['list']['keyword'][$i] . '%\') or ';
							}
							else
								$this->_keyword .= ' (k_desc2 like \'%' . $this->_config['list']['keyword'][$i] . '%\')';
		...
		if(strpos($_SERVER['REQUEST_URI'], 'fairevent') !== false)
		{
			...
			$list = $this->_db->get_target_event_for_fairevent($this->_keyword, $view);
			
		}
		else
		{
			if($this->_config['show_limit'] != 0){
				$list = $this->_db->get_target_event_list_for_list($this->_keyword, $this->_config['show_limit']);
			}
			else 
				$list = $this->_db->get_target_event_list_for_list($this->_keyword);
		}
		
		if($_SERVER["REMOTE_ADDR"] == "115.79.57.237"){
			//echo $this->_config['end_date'];
		}
		foreach ($list as $row) {
			...
			*/
			...
			$c_desc = $row['k_desc1'];
			/*if(strpos($row['k_desc1'], 'https'))
			{
				$c_desc = $row['k_desc1'];
			}
			else if (strpos($row['k_desc1'], 'http') == 0 && strpos($row['k_desc1'], 'http') == 0){
				$c_desc = str_replace('http','https',$row['k_desc1']);
			}
			*/
			//$c_desc = str_replace('http','https',$row['k_desc1']);
			...
			if (!$this->pc_view && $this->_header_obj->computer_use() === false && $_SESSION['pc'] != 'on') {
				// spの時
				$add_margin_left = "";
				$add_detail_margin = "";
			...
			$this->_calendar[$year.'-'.$month.'-'.$day] .= '<img src="https://'.$_SERVER['SERVER_NAME'].'/images/sa05.jpg">';
                        if($this->_config['use_mode'] == 'widget'){
                            $c_msg  = '<tr><td nowrap class="seminar_timeandplace" style="vertical-align:middle; width:20%; text-align:center">'.$row['starttime'].'～ <img src="https://'.$_SERVER['SERVER_NAME'].'/images/sa05.jpg">' . $this->_translate_city($row['place']) . '</td><td nowrap class="seminar_button" style="vertical-align:top; width:10%; text-align:center' . $add_text_align . '">'.$c_btn.'</td>';
                        }else{
                            $c_msg  = '<tr><td nowrap class="seminar_timeandplace" style="vertical-align:top; width:20%;">'.$row['starttime'].'～ <img src="https://'.$_SERVER['SERVER_NAME'].'/images/sa05.jpg">' . $this->_translate_city($row['place']) . '</td><td nowrap class="seminar_button" style="vertical-align:top; width:10%;' . $add_text_align . '">'.$c_btn.'</td>';
                        }
			$c_msg .= $add_colspan;
			$c_msg .= $c_img;
			$detail_open = 'none';
	...
	private function _show_calendar()
	{
		...
		echo $this->_get_calendar_know();
                ...
                //JN:add code
                echo '<input type="hidden" name="jn_day_selected" value="' . $this->jn_day_selected . '" />';
                echo '<input type="hidden" name="jn_day_limit" value="' . $this->jn_day_limit . '" />';
                //JN:end
		...
		if ($this->_config['calendar']['calendar_icon_active'] == 'active' || $this->_config['calendar']['calendar_icon_active'] == 'yes' || $this->_config['calendar']['calendar_icon_active'] == 'on' || $this->_config['calendar']['calendar_icon_active'] == 'true') {
			...
			<div class="orange-solid">
				...
				<span style="margin-left:50px;"><img src="/css/images/hoshi.png" alt="osusume" />&nbsp;&nbsp;おススメのセミナーです</span>
				<span style="margin-left:140px;"><span class="_chumoku">注目</span>&nbsp;&nbsp;注目のセミナーです</span><br/><br/>
			</div>';
		}
...
';*/
	 ...
	 * リストの表示
	 */
	function _show_list_testabc($target_seminar_id = "")
	{
		if($this->_event_count > 0) echo $this->_event_count;
		else echo 'khong co cai nao het';
	}
	private function _show_list($target_seminar_id = "")
	{
		...
		$isPc = true;
		if (!$this->pc_view && $this->_header_obj->computer_use() === false && $_SESSION['pc'] != 'on') $isPc = false;
	...
	/**
	 * get count event seminar
	 * @param None
	 * @return number
	 */
    public function tp_getEventCount(){
...
<div id="yoyakuform" style="display:none; margin:15px 20px 15px 20px; font-size:10pt; cursor:default;">
	...
	<div id="form0" style="">
		<div style="font-size:12pt; font-weight:bold; margin:0 0 5px 0;">セミナー　予約フォーム</div>
		<div style="font-size:9pt; font-weight:bold; margin:5px 0 5px 0; border: 1px dotted navy;;">
			セミナーのご予約に際し、ログインして下さい。
		</div>
		<div style="font-size:9pt;">
			メンバー様専用セミナーの予約の為にログインします。<br>
			ご登録いただいた、メールアドレスとパスワードを入力してください。<br><br>
		</div>
		<div>
			<form name="yoyaku_login" id="yoyaku_login">
				<table style="width:560px;margin-bottom: 30px;font-size: 10pt;">
					<tr>
		...
		<div id="div_wait_login" style="display:none;">
			<img src="https://'.$_SERVER['SERVER_NAME'].'/images/ajaxwait.gif">
			&nbsp;予約処理中です。しばらくお待ちください。&nbsp;
			<img src="https://'.$_SERVER['SERVER_NAME'].'/images/ajaxwait.gif">
		</div>
		<div style="margin-top:10px;">
		...
		<div style="font-size:9pt; margin:15px; padding: 20px; border: 1px solid black;">
			パスワードが解らない、登録時のメールアドレスを忘れた場合は<a href="/member">こちら</a>
		</div>
	</div>
	<div id="form1" style="">
		<div style="font-size:12pt; font-weight:bold; margin:0 0 8px 0;">セミナー　予約フォーム</div>
		...
		<div style="font-size:9pt; font-weight:; text-align:left; margin:10px 0 10px 20px;">
			...
			&nbsp;<br/>
			３．　Gmailアドレスをご利用の場合、<br/>
			　　　仮予約時の予約確認のメールが迷惑メールフォルダに入ってしまう可能性がございます。<br/>
			　　　メールが届かない場合は一度迷惑メールフォルダをご確認ください。<br/>
			&nbsp;<br/>
			４．　Ｈｏｔｍａｉｌ、Ｙａｈｏｏメールなどをご利用の場合、予約確認のメールが遅れて<br/>
			　　　到着する場合があります。時間をおいてから受信確認を行うようにしてください。<br/>
			&nbsp;<br/>
			５．　予約確認メールが届かない場合、toiawase@jawhm.or.jp までご連絡ください。<br/>
			　　　なお、迷惑フォルダ等に分類される場合もありますので、併せてご確認ください。<br/>
			&nbsp;<br/>
			<p class="regulative_seminar"> 
				最近、会場を間違えてご予約される方が増えております。<br/>
				セミナー内容・会場・日程等を十分ご確認の上、ご予約頂けますようお願い申し上げます。<br/>
				<span>
					・セミナー予約のキャンセルの際には必ず前日までにキャンセル手続きをお願い申し上げます。&nbsp;<br/>
					・当日のキャンセルや、キャンセル手続き無くセミナーへの欠席が続いた場合、その後のご予約をご遠慮する場合もございます。
					多くの方に情報提供をさせていただくためのセミナーとなりますのでご理解ください。&nbsp;<br/>
				<span>
			</p>
		</div>
		<div style="margin-top:10px;">
...
';
		$output .= '
				<div class="form-jawhm">
					<div class="tab-form">
						<h1>セミナー名</h1>
						<div class="info_seminar" id="form_title"></div>
						<span onClick="btn_cancel();" class="fa fa-times"></span>
					</div>
				</div>
				<form name="form_yoyaku" id="form_yoyaku" class="form-jawhm">
				
                <div class="clear"></div>
				<input type="hidden" name="セミナー名" id="txt_title" value="">
				<input type="hidden" name="セミナー番号" id="txt_id" value="">
                <div id="msg_hajimete" class="tab-form">
                    <p class="left red">ご注意</p>
                    <div class="right red">初めてセミナーご参加される場合は、<br><a href="https://'.$_SERVER['SERVER_NAME'].'/seminar/seminar_01" target="_blank">初心者向けセミナー</a>へのご予約をお願いします。
                    </div>
                </div>
		...
		$output .= '
	<div id="seminarbtn" class="seminar_btn">
		<div id="div_wait" style="display:none;">
			<img src="https://'.$_SERVER['SERVER_NAME'].'/images/ajaxwait.gif">
			&nbsp;予約処理中です。しばらくお待ちください。&nbsp;
			<img src="https://'.$_SERVER['SERVER_NAME'].'/images/ajaxwait.gif">
		</div>
		<button id="btn_soushin" class="button_submit jawhm-button" value="予約する(無料)" onClick="btn_submit();"><a href="#">予約する</a></button>
	</div>	
	</div>
</div>
';
...
jQuery(function($) {
	...
	private function _generate_calendar()
	{
		...
		if (!empty($this->_config['start_date']) && !empty($this->_config['end_date'])) {
			...
			//}
			//echo $this->_event_count; exit;
		}else{
                    //JN:block
                    //echo 'Hello';
                    //echo $this->_year;
                    //echo $this->_month;
                    //echo $this->jn_day_selected;
                    //JN:end    
			$ret .= calender_show($this->_calendar, $this->_year, $this->_month, $this->_translate_city($this->_place_name), $this->_place_name, $this->_keyword, $this->_checked_countryname, $this->_checked_know, $this->_get_navigation_used(), $this->_selected_day, $this->_config['calendar']['calendar_title_active'], $this->_config['start_date'], $this->_config['end_date'], $this->jn_day_selected, $this->jn_day_limit); //JN:add para:$this->jn_day_selected, $this->jn_day_limit
		}
...
';
		...
		if( $_SERVER['REMOTE_ADDR'] == "1.52.108.221")
		{
			//print_r($this->_calendar_msgs);exit;
		}
	...
	private function _check_event()
	{
		if (!empty($this->_num))
		{
			...
			$this->_day   = $this->_selected_event_info['ddd'];
                        //JN:add code
                        $this->jn_day_selected = $this->_selected_event_info['ddd'];
                        //JN:end
			...
			if (!empty($_POST['place-name'])) {
				...
				//minhquyen -- redirect when found "date" -- 20180117
				if(strpos($_SERVER['REQUEST_URI'], "date") != false)
				{
					$mobileredirect = array();
					$mobileredirect = explode("/", $_SERVER['REQUEST_URI']);
					$mobileredirect = "/seminar/ser/".$mobileredirect[3]."/".$mobileredirect[4]."/".$mobileredirect[5];
