<?php
	...
	function calender_show($year, $month,$placename_in_japanese='東京', $place_name='tokyo', $keyword='place=tokyo', $checked_countryname = 1, $checked_know = 1, $navigation_used = 0, $selected_day = 0 )	
	{
		...
		if($limit_date['half_month'] && $limit_date['ending_date_mm'] != $limit_date['beginning_date_mm'])
		{
			if($limit_date['ending_date_yy'] == $limit_date['beginning_date_yy'])
				...
				echo '<div id="place-date-seminar">'.$limit_date['beginning_date_yy'].'年'.$beginning_date_mm.'月'.'-'.$ending_date_yy.'年'.$ending_date_mm.'月&nbsp;'.$placename_in_japanese.'会場'.'</div>';
		}
		else
		{	
	...
	function check_limit_days_to_display($year,$month,$navigation_used,$selected_day=0)
	{
		...
		$date_limit_ending = 31; //used to be $num+28
			...
			while($row = $rs->fetch(PDO::FETCH_ASSOC))
			{
				...
				else
				{
					...
					if (in_array($flag, $array_flay)) 
					{
						...
						$list_flag .= '<span id="flag_'.$flag.'"></span>';
				...
				$listing_data .= "<div id=\"description_seminar\" class=\"".$fullybooked."\" ".$style_description.">".$decription."</div>"
					. "<div id=\"name_seminar\" class=\"".$fullybooked."\" ".$style_name." onmouseover=\"Tip('".$text_tooltip."', BGCOLOR, 'WHITE', TITLE, '".$seminar_booked.$seminar_name."', TITLEBGCOLOR, '".$title_bg_color."', TITLEFONTCOLOR, '".$title_color."', BORDERCOLOR, '".$color."', BORDERSTYLE, '".$border_style."', PADDING, 9, WIDTH, -300)\" onmouseout=\"UnTip()\">".$seminar_name."</div>";
	...
	function calender_list($year, $month, $place_name)	
	{
		...
		for($i=1;$i<=62;$i++) // 62 is to display two months results
		{
			...
			if (@$cal[date('Ynj', $print_today)] <> '')	
			{
				...
				print '<table class="table_list" >'.$cal_msg[date('Ynj', $print_today)].'</table>';
	...
	function where_country($checked_countryname)
	{
		...
		if (substr_count($checked_countryname, 1) == 1 || $checked_countryname == '0')	//all
		{
			$where_country = '';
		}
		else
		{
			if(substr_count($checked_countryname, 7) == 1 )	//others			
			{
	...
	//			$where_country .= '  and k_desc2 not like  \'%フランス%\' ';
			...
			{
				if(substr_count($checked_countryname, 2) == 1 )
				{
					$val='オーストラリア';
				...
				if(substr_count($checked_countryname, 3) == 1 )
				{
					$val='ニュージーランド';
					$where_country .= ' or k_title1 like \'%'.$val.'%\' ';
					$where_country .= ' or k_desc1 like \'%'.$val.'%\' ';
				...
				if(substr_count($checked_countryname, 4) == 1 )
				{
					$val='カナダ';
					$where_country .= ' or k_title1 like \'%'.$val.'%\' ';
					$where_country .= ' or k_desc1 like \'%'.$val.'%\' ';
				...
				if(substr_count($checked_countryname, 5) == 1 )
				{
					$val='イギリス';
					$where_country .= ' or k_title1 like \'%'.$val.'%\' ';
					$where_country .= ' or k_desc1 like \'%'.$val.'%\' ';
				...
				if(substr_count($checked_countryname, 6) == 1 )
				{
					$val='フランス';
					$where_country .= ' or k_title1 like \'%'.$val.'%\' ';
					$where_country .= ' or k_desc1 like \'%'.$val.'%\' ';
	...
	function where_know($checked_know)
	{
		...
		if (substr_count($checked_know, 1) == 1 || $checked_know == '0')	//all
		{
			$where_know = '';
		}else
		{
			if(substr_count($checked_know, 2) == 1 )
			{
				$val='初心者向け';
			...
			if(substr_count($checked_know, 3) == 1 )
			{
				$val='懇談';
			...
			if(substr_count($checked_know, 4) == 1 )
			{
				$data = '比較';
			...
			if(substr_count($checked_know, 5) == 1 )
			...
			if(substr_count($checked_know, 6) == 1 )
			{
	...
	//--------------------------------------------------------------------
	function get_days_list ($place_name,$keyword)
	{
		// イベント読み込み
		global $cal;
		global $cal_msg;
		...
		try {
			...
			while($row = $rs->fetch(PDO::FETCH_ASSOC)){
				...
				{									
					if ($row['free'] == 1)
					{
						...
						else
						{
							...
							{
								if ($row['booking'] >= $row['pax'])	
									$c_btn	= '<input class="button_yoyaku" type="button" value="キャンセル待ち" onclick="fnc_yoyaku(this);" name="['.translate_city($row['place']).'A]'.$title.'" uid="'.$row['id'].'">';
								else
									$c_btn	= '<input class="button_yoyaku" type="button" value="メンバー専用予約" onclick="fnc_yoyaku(this);" name="['.translate_city($row['place']).'A]'.$title.'" uid="'.$row['id'].'">';
							}
						}
					}
					else
					{					
						...
						else
						{
							if ($row['booking'] >= $row['pax'])	
								...
								$c_btn	= '<input class="button_yoyaku" type="button" value="予約" onclick="fnc_yoyaku(this);" name="['.translate_city($row['place']).'A]'.$title.'" uid="'.$row['id'].'">';
						}
					}
				...
				if ($row['place'] == 'event')	{
					$c_msg  = '<tr><td nowrap style="vertical-align:top;" colspan="3"><div class="square_color" style="background-color:'.$color.'; '.$bdcolor.'">&nbsp;</div><span class="starttime">'.$row['starttime'].'～ </span>'.translate_city($row['place']).'会場';
				}else{
