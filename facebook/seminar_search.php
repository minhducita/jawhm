...
print '<dl id="accordion">';

	for($i=$day;$i<=$day + 150;$i++){
		$print_today = mktime(0, 0, 0, $month, $i, $year);
		if (@$cal[date('Ynj', $print_today)] <> '')	{
		//	print '<div style="width:490px; font-size:12pt; margin: 5px 0 10px 0;" id="'.date('Ymd', $print_today).'">';
		//	print '<div style="border-left:4px solid red; border-top:1px solid red; padding:5px 0 5px 10px;">';
		//	print date('n月j日 ('.$yobi[date('w', $print_today)].')', $print_today).'　<a target="_top" href="http://www.jawhm.or.jp/seminar.html#'.date('Ymd', $print_today).'">このセミナーに興味がある場合はココ！！</a>';
		//	print '</div>';
		//	print '<table style="margin-left:14px; table-layout:fixed;">'.$cal_msg[date('Ynj', $print_today)].'</table>';
		//	print '</div>';
			print $cal_msg2[date('Ynj', $print_today)];
...
<?
	...
	try {
		...
		$db->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
		$db->query('SET CHARACTER SET utf8');
//		$rs = $db->query('SELECT id, year(hiduke) as yy, month(hiduke) as mm, day(hiduke) as dd, title, memo, place, k_use, k_title1, k_desc1, k_stat FROM event_list WHERE k_use = 1 and free=0 AND hiduke >= "'.date("Y/m/d",strtotime("-1 week")).'"  ORDER BY hiduke, id');
		...
		while($row = $rs->fetch(PDO::FETCH_ASSOC)){
			...
			if ($row['place'] == 'tokyo')	{
				...
				$c_msg  = '<tr><td nowrap style="vertical-align:top;">'.$row['starttime'].'～ <img src="../images/sa05.jpg">東京会場</td><td nowrap style="vertical-align:top;"></td>';
...
$cal_msg2[$year.$month.$day] .= '
     <dt><a href=""><u>'.$month.'月'.$day.'日 '.$row['starttime'].'～</u> <img src="../images/sa05.jpg">東京会場<br/>'.$row['k_title1'].'</a></dt>
     <dd><a target="_top" href="http://www.jawhm.or.jp/seminar.html#'.$year.substr('00'.$month,-2).substr('00'.$day,-2).'">ご予約はコチラからどうぞ</a><br/>&nbsp;<br/>'.$c_title.nl2br($c_desc).'</dd>
';
			...
			if ($row['place'] == 'osaka')	{
				...
				$c_msg  = '<tr><td nowrap style="vertical-align:top;">'.$row['starttime'].'～ <img src="../images/sa01.jpg">大阪会場</td><td nowrap style="vertical-align:top;"></td>';
				$c_msg .= '<td>'.$row['k_title1'].'</td></tr><tr><td colspan="4">';
				$c_msg .= ''.$c_img.'<div class="open" style="margin-left:150px;">セミナー詳細はココをClick!!</div>';
				$c_msg .= '<div class="det" style="margin:5px 0 10px 50px; padding: 5px 0 13px 12px; display:none; border-left:1px dotted navy; border-bottom: 1px dotted navy;">'.$c_title.nl2br($c_desc).'</div></td></tr>';
				$cal_msg[$year.$month.$day] .= $c_msg;
...
$cal_msg2[$year.$month.$day] .= '
     <dt><a href=""><u>'.$month.'月'.$day.'日 '.$row['starttime'].'～</u> <img src="../images/sa05.jpg">大阪会場<br/>'.$row['k_title1'].'</a></dt>
     <dd><a target="_top" href="http://www.jawhm.or.jp/seminar.html#'.$year.substr('00'.$month,-2).substr('00'.$day,-2).'">ご予約はコチラからどうぞ</a><br/>&nbsp;<br/>'.$c_title.nl2br($c_desc).'</dd>
';
			}
			...
			if ($row['place'] == 'fukuoka')	{
				...
				$cal[$year.$month.$day] .= '<img src="../images/sa05.jpg">';
				$c_msg  = '<tr><td nowrap style="vertical-align:top;">'.$row['starttime'].'～ <img src="../images/sa03.jpg">福岡会場</td><td nowrap style="vertical-align:top;"></td>';
				$c_msg .= '<td>'.$row['k_title1'].'</td></tr><tr><td colspan="4">';
				$c_msg .= ''.$c_img.'<div class="open" style="margin-left:150px;">セミナー詳細はココをClick!!</div>';
				$c_msg .= '<div class="det" style="margin:5px 0 10px 50px; padding: 5px 0 13px 12px; display:none; border-left:1px dotted navy; border-bottom: 1px dotted navy;">'.$c_title.nl2br($c_desc).'</div></td></tr>';
				$cal_msg[$year.$month.$day] .= $c_msg;
...
$cal_msg2[$year.$month.$day] .= '
     <dt><a href=""><u>'.$month.'月'.$day.'日 '.$row['starttime'].'～</u> <img src="../images/sa05.jpg">福岡会場<br/>'.$row['k_title1'].'</a></dt>
     <dd><a target="_top" href="http://www.jawhm.or.jp/seminar.html#'.$year.substr('00'.$month,-2).substr('00'.$day,-2).'">ご予約はコチラからどうぞ</a><br/>&nbsp;<br/>'.$c_title.nl2br($c_desc).'</dd>
';
			}
			...
			if ($row['place'] == 'sendai')	{
				...
				$c_msg  = '<tr><td nowrap style="vertical-align:top;">'.$row['starttime'].'～ <img src="../images/sa02.jpg">仙台会場</td><td nowrap style="vertical-align:top;"></td>';
				$c_msg .= '<td>'.$row['k_title1'].'</td></tr><tr><td colspan="4">';
				$c_msg .= ''.$c_img.'<div class="open" style="margin-left:150px;">セミナー詳細はココをClick!!</div>';
				$c_msg .= '<div class="det" style="margin:5px 0 10px 50px; padding: 5px 0 13px 12px; display:none; border-left:1px dotted navy; border-bottom: 1px dotted navy;">'.$c_title.nl2br($c_desc).'</div></td></tr>';
				$cal_msg[$year.$month.$day] .= $c_msg;
...
$cal_msg2[$year.$month.$day] .= '
     <dt><a href=""><u>'.$month.'月'.$day.'日 '.$row['starttime'].'～</u> <img src="../images/sa05.jpg">仙台会場<br/>'.$row['k_title1'].'</a></dt>
     <dd><a target="_top" href="http://www.jawhm.or.jp/seminar.html#'.$year.substr('00'.$month,-2).substr('00'.$day,-2).'">ご予約はコチラからどうぞ</a><br/>&nbsp;<br/>'.$c_title.nl2br($c_desc).'</dd>
';
			}
			...
			if ($row['place'] == 'okinawa')	{
				...
				$cal[$year.$month.$day] .= '<img src="../images/sa02.jpg">';
				$c_msg  = '<tr><td nowrap style="vertical-align:top;">'.$row['starttime'].'～ <img src="../images/sa02.jpg">沖縄会場</td><td nowrap style="vertical-align:top;"></td>';
				$c_msg .= '<td>'.$row['k_title1'].'</td></tr><tr><td colspan="4">';
				$c_msg .= ''.$c_img.'<div class="open" style="margin-left:150px;">セミナー詳細はココをClick!!</div>';
				$c_msg .= '<div class="det" style="margin:5px 0 10px 50px; padding: 5px 0 13px 12px; display:none; border-left:1px dotted navy; border-bottom: 1px dotted navy;">'.$c_title.nl2br($c_desc).'</div></td></tr>';
				$cal_msg[$year.$month.$day] .= $c_msg;
...
$cal_msg2[$year.$month.$day] .= '
     <dt><a href=""><u>'.$month.'月'.$day.'日 '.$row['starttime'].'～</u> <img src="../images/sa05.jpg">沖縄会場<br/>'.$row['k_title1'].'</a></dt>
     <dd><a target="_top" href="http://www.jawhm.or.jp/seminar.html#'.$year.substr('00'.$month,-2).substr('00'.$day,-2).'">ご予約はコチラからどうぞ</a><br/>&nbsp;<br/>'.$c_title.nl2br($c_desc).'</dd>
';
			}
...
<?
	if ($cnt == 0)	{
		...
		print '<div style="font-size:12pt; margin-left:10px;">';
		print '現在、'.$cnt.'件のセミナーが予定されています。<br/>';
		print '気になるセミナーをクリックしてください。詳細が表示されます。<br/>';
		print '</div>';
	}
?>
...
<table style="font-size:12pt; width:480px;">
	<tr>
...
<div style="width:500; float:clear;">
<?
	calender_list();
?>
</div>
